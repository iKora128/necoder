...
### Added
- Mac 標準のテキスト編集キーを一式（エディタと AI の入力欄で共通）: ⇧⌘← / ⇧⌘→・⇧Home / ⇧End
  = 行頭/行末まで選択、⇧⌘↑ / ⇧⌘↓ = 文書の先頭/末尾まで選択、⌘⌫ = 行頭まで削除、
  ⌃K = 行末まで削除（行末では行結合）、⌥Delete = 次の単語を削除、⌃N / ⌃P / ⌃F / ⌃B / ⌃D / ⌃H
  = emacs 風の移動と削除。Windows は VSCode の綴り（Ctrl+Shift+Home 等）に落とし、衝突する
  emacs 風は付けない

### Fixed
- エディタ本文のクリック位置とキャレットのずれ。ヒットテストが描画と別のフォント（OS 既定の
  プロポーショナル）で幅を測っていたため、日本語と ASCII が混ざる markdown の行ではクリック点から
  全角 3 文字ほど離れた位置にキャレットが置かれ、ドラッグ選択・hover の位置も同じだけずれていた。
  描画に使ったフォントで測るようにした
- Remote SSH のプロジェクトに切り替えるとアプリが数十秒無反応になることがある問題。切替ハンドラが
  UI スレッドから remote host のファイル一覧・タブ復元・変更監視を同期で呼んでおり、スリープ復帰
  直後のように SSH の再接続が要る場面ではその待ち時間がそのまま操作不能時間になっていた。これらを
  背景処理に移し、宛先チップは即座に、候補ファイルとタブは読めた順に追いつくようにした
- スリープ復帰後の Remote SSH 再接続が長引く問題。`ssh -O check` は多重化 master の生死しか見ず、
  TCP だけ死んだ master を「生きている」と答えるため、その上に張った接続が黙って固まっていた。
  ハンドシェイクを 10 秒で見切って master ごと張り直すようにし、到達不能なホストへの再試行間隔も
  5 秒から最大 60 秒へ広げた
- Remote SSH のプロジェクトを開いたまま終了すると、次の起動でウィンドウが出るまでに接続を待って
  いた問題。到達できないホストが混じっていると、その分だけ起動が遅れていた。前回保存した情報だけで
  レールを組み立てて先にウィンドウを出し、接続と中身の読み込みは背景で追いつくようにした
- スリープ復帰後の Remote SSH で、切れた接続に気付くまでに最大 30 秒かかっていた問題。生存確認
  （Ping）の待ち時間を 5 秒にし、張り直しが決まった時点でその接続に乗っていた要求を即座に
  失敗させて新しい接続でやり直すようにした（以前は要求ごとのタイムアウトまで待っていた）。
  時間切れでも受信が進んでいれば「混んでいるだけ」とみなして切らない（低速回線で大きなファイルを
  転送中に切って再送を繰り返すのを防ぐ）
- ハンドシェイクが時間切れではなく即座に閉じたときにも ControlMaster を作り直し、同じ接続に
  乗っていた端末を巻き添えにしていた問題。remote-server 側の起動失敗では master を触らない
- Remote SSH で、フォルダの展開・折り畳み・リネーム・変更監視のイベント・ブランチ切替のたびに
  アプリが SSH の往復ぶん止まっていた問題。ツリーの読み取りを背景へ移し、折り畳みと読み込み済み
  フォルダの展開は即座に映るようにした
- Remote SSH で、開いているファイルが外で変わったときの照合と再読込、端末・LSP・エージェントの
  起動、エージェントのターン開始/終了時の HEAD 取得が UI スレッドで接続を待っていた問題。端末と
  LSP の SSH セッションは master が無ければ自分で繋ぐようにし（`ControlMaster=auto`）、起動仕様を
  組む段階ではネットワークに触らないようにした
- Remote SSH の接続が切れている間、操作するほど遅くなっていた問題。到達不能なホストへの再接続は
  1 回で十数秒かかるのに、待たされていた操作がそれぞれ最初からやり直していた。待っている間に
  起きた失敗はそのまま受け取るようにし、後から来た操作（＝ユーザーの再試行）だけが繋ぎに行く
- アップデートの確認が起動 10 秒後の 1 回きりだった問題。necoder を開いたままだと、その後に出た
  版は開き直すまで見えなかった。6 時間ごとに見直すようにした。経過は壁時計で測るので、スリープを
  挟んでも「寝ていた時間」がちゃんと数えられる
