...
更新版の歯車メニューにある「スマホ連携」から、ターミナルなしでQRを発行できます。
発行時に開いている全プロジェクトを共有します。画面からURLのコピー、QRの非表示、再発行が可能です。
QRは5分で自動的に表示を終了します。「隠す」だけでは未使用QRは期限まで有効です。
初回ホスト設定と Node.js 22+ は必要です。設定画面で発行したQR画像やURLをファイルやログへ出力しません（接続鍵の保護ファイル保存は従来どおり行います）。
...
Push は 2 種類です: 承認・質問を待っている時（「エージェントが回答を待っています」）と、走っていたスレッドが終わった時（「エージェントが作業を終えました」・承認 / 質問待ちや切断は含めない）。本文は種類だけで、会話・コード・プロジェクト名を含めません（端末の表示文は Service Worker の固定文）。通知の配信は OS と push service 次第であり、保証されません。
