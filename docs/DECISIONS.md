...
- **Chat モード = 「セッション設定のプリセット」、artifact = 「エージェントが書いたファイル + 既存プレビュー」（2026-09-18・本人）** —
  Editor / Fleet に並ぶ第 3 のモード。claude.ai や ChatGPT のような、プロジェクトに紐づかない会話の場。
  **新しいチャットエンジンは作らない**。UI は `agent_panel` の transcript をそのまま使い、変えるのは ACP セッションの作り方だけ。
  **なぜ成立するか（2026-09-17 実験）**: Claude Code が「ガンガン実装する」のはモデルの性格ではなく、長いシステムプロンプトと
  Bash などの道具を全部持っていることが原因。システムプロンプトを丸ごと差し替え、ツールを Write / Edit / Read に絞って空ディレクトリで走らせたところ、
  「Rust の所有権って何？」にはテキストだけで答え（ファイル 0 個）、「ポモドーロタイマー作って」には `artifacts/pomodoro.html` を 1 個だけ書いて 2 文で終わった。
  `claude-agent-acp` は同じ SDK オプションを `_meta.systemPrompt`（文字列 = 置換）と `_meta.claudeCode.options.tools` で受ける（`acp-agent.js` で確認）。
  **確定 8 点**:
  ① **プリセットの中身** = 会話ごとの永続のフォルダ（cwd）/ 自前の短いシステムプロンプト / `tools` = Read・Write・Edit・Glob・Grep・WebSearch・WebFetch（Bash は入れない）/
  `settingSources` を空にして user・project・local 設定の持ち込みを止める / necoder が `session/new` に載せる `mcpServers` も空（ユーザーが明示的に選んだものだけ）。
  `allowedTools` は**使用可能ツールの限定ではなく自動承認の指定**なので、限定には使わない。`tools` だけでは MCP は制限されないので `settingSources` と `mcpServers` を別に閉じる。
  ② **プロンプトには行動規則を書く**（「会話アシスタントです」では足りない）: 質問・相談には会話で答える。成果物の作成・変更を求められた場合だけファイルを編集する。
  成果物は作業フォルダの `artifacts/` に保存し、**既存成果物やユーザーが渡したファイルの修正は同じファイルを更新する**（ファイル方式が成立する条件）。
  `artifacts/` を挟む理由: このフォルダには成果物以外（会話の書き出し・作業用ファイル）も置くので「この中だけが artifact」と決まる / 表示の隔離の配信範囲を絞れる / 実験で確認したのはこの形。直下に置く案は同日中に取り下げた。
  ③ **cwd は作業場所であって隔離ではない**。書き込み先の制限は necoder 側の `handle_permission_request` が持つ。エージェント側の設定に頼らない。
  **保存場所は書類フォルダの `necoder/<YYYY-MM-DD 最初のメッセージの先頭>/`**（同日改訂・本人の指摘「書類に necoder を作るのがよいのでは」）。
  当初案の `~/.necoder/chats/<thread id>/` は採らない — 隠しフォルダに成果物を置くとユーザーが自分の書類を見つけられない。成果物はユーザーのもので、necoder を消しても残るべき。
  フォルダ名は作成後に変えない（Claude Code はセッションを cwd のパス文字列で引くので、変えると `session/load` が前回を見つけられない）。
  作るのは最初の送信時、空のまま終わったフォルダは消す（相談だけのチャットで書類が散らからない）。履歴は既存の DB のまま。
  macOS は `~/Documents`（初回に TCC の許可ダイアログ）、Windows は Known Folder（OneDrive へのリダイレクトがあるので `%USERPROFILE%\Documents` を決め打ちしない）、Linux は `XDG_DOCUMENTS_DIR`。設定 `chat.directory` で変更可。
  **ファイルを渡す = 触ってよいと伝える**: ドロップしたファイル・フォルダは読める。書き込みは初回だけ確認（チェックポイントで戻せる）。それ以外の場所への書き込みは自動で拒否。詳細は `CHAT.md` §2〜§3。
  ④ **再開にも同じ設定を載せる**。necoder は前回のセッション id があれば `session/load` を使い、`session/new` とは別経路。
  アダプタは `loadSession` でも `params._meta` を読むので、necoder が両方の経路に `_meta` を載せれば保たれる。プリセットの種類とフォルダの場所はスレッドの行に保存する。
  ⑤ **artifact はファイル**。表示は既存の HTML プレビュー（`webview_view`）と Markdown プレビューを使い、編集・保存・再表示の仕組みを共有する。
  プレビューチップはツール呼び出しの構造化されたパス（ACP の `locations`）と実在確認で出し、文言は解析しない。最初は単体 HTML と Markdown に限定（ビルド環境も開発サーバも不要）。
  **比較して採らなかった方式**: テキストの目印を解析する方式（LibreChat の `:::artifact`）はファイルとの二重管理が生まれる。
  `create_artifact` / `publish_artifact` の MCP ツールは、タイトル・公開対象・版を明示したくなった時に検討する。
  MCP はツールを届ける手段であって artifact の成立条件ではなく、**今ほしい情報はファイルパスと既存のツール通知で足りる**。
  Claude Code 公式の Artifacts は claude.ai への公開と認証に依存する機能なので、ローカル表示の基盤には採らない。
  ⑥ **自動再読込はターン終了時に 1 回**。ツール呼び出しごとに再読込するとタイマーなどのページ状態が消える。
  外部変更の経路（`after_reload`）がプレビューを再読込していない現状は、Chat と無関係に直す。
  ⑦ **生成 HTML の表示は隔離する**。custom protocol だけでは完成しない。配信できるファイルの範囲（そのチャットの `artifacts/` のみ）・外部通信・画面遷移・
  ネイティブ機能へのアクセスを合わせて制御する。Chat を日常で使い始める前に入れる。
  ⑧ **「重くならない」は条件付き**。追加コストは起動中のエージェントと WebView。非表示の WebView も evict（既定 15 分）までインスタンスを保持するので、
  Chat は開くまでエージェントを起動しない・回収弁を共有する、を守る。プロンプト短縮による初速・総コストの改善は**実測してから**言う。
  **制約**: プロンプトの差し替えは `claude-agent-acp` 固有の `_meta` で ACP 標準ではない。最初は **Chat は Claude 限定**。
  **色**: Chat はプロジェクトに属さないので額縁はプロジェクト色を持たない（無彩色）。会話 = スレッドなのでスレッド色は持つ（宛先チップ・入力枠・送信ボタン・一覧の行頭）。
  実装順は ROADMAP M16（C0〜C6）。保存場所・ファイルの扱い・機能一覧（P0〜P2）の正は `CHAT.md`、見た目の提案は `mock/chat.html`。
  **実装して決まったこと（2026-09-19 追記）**:
  ⑨ **claude.ai アカウントのコネクタは環境変数でしか止まらない**。`settingSources: []` と `strictMcpConfig` を立てても、
  アカウントに繋いだコネクタ（Figma・Google Calendar 等）は自動で読み込まれ、170 個超のツール定義が最初のターンから文脈を
  11 万トークン食った。Chat のプリセットは `ENABLE_CLAUDEAI_MCP_SERVERS=false` をエージェントのプロセスへ渡す（5,202 トークンに落ちた）。
  ①の「持ち込みを止める」は `_meta` だけでは完結しない、が教訓。
  ⑩ **許可は必ず「今回だけ」で答える**。エージェントに「常に許可」を返すと、エージェント側が自分の権限モードを切り替えて
  以後のリクエストが necoder へ来なくなる＝裁定を手放すことになる。「このチャットでは以後許可」は necoder が覚え、
  エージェントには今回だけの許可として返す。選択肢は間引かない（応答は並びの添字なので、間引くと別の選択肢を選んだことになる）。
  ⑪ **Chat の session は `projects` に混ぜない**。レールの枠と同じ添字で並ぶ `sessions` に入れると、レール・⌘1..9・永続化・
  Fleet の全部に「枠の無い session」の例外が要る。別に 1 本持ち、Chat 中は `Deref` がそれを返す形にした
  （「アクティブな session」を相手にしている既存のコードがそのまま Chat で動く）。
  ⑫ **隔離で許す CDN は 4 つ**（cdnjs / jsdelivr / unpkg / Google Fonts・スクリプトとスタイルと書体だけ）。`connect-src 'none'` なので
  ページは何も送れない。全部閉じるとチャートや Tailwind を使う artifact が作れず、開けすぎると外部画像がビーコンになる。
  Chat 用プロンプトにも同じ一覧を書いて、エージェントが動かない成果物を作らないようにした。
  **重さの正体と対処（2026-09-19 追測）**:
  ⑬ **エージェントは 1 本 350〜700 MB**（`npm exec` のラッパー 48MB + アダプタの node 65MB + claude 本体 230〜570MB）で、
  一度起こすとスレッドを閉じるまで生きる。開きっぱなしのスレッド 20 本の窓で**エージェント合計 7.8GB / necoder 本体 291MB**。
  これは Chat で増えた重さではなく Editor / Fleet も同じ。**idle メモリ予算（122MB）はアプリ本体の話で、
  エージェントを数えると 2 桁違う**ことを明示しておく。対処は 2 つ:
  ① 使っていないエージェントを止める（`agent_idle_stop_minutes` 既定 15 分・Chat は 10 分）。
  **会話を引き継げるスレッドだけ**（`loadSession` の広告 + セッション id）を対象にし、次の送信で `session/load` が戻す。
  常時回る時計は持たず、ターン終了ごとに一発のタイマーを仕掛ける。
  ② `npx -y <pkg>@<ver>` をやめて、同じ版が npx のキャッシュに在ればその実行ファイルを直接起こす
  （`npm exec` の親プロセス 48MB/本と、起動のたびの依存解決 1 秒が消える）。**版がちょうど一致する時だけ** —
  「上限以下なら何でも」にすると、レジストリが版を上げても古いキャッシュを使い続けて更新が止まる。
  ⑭ **claude.ai のコネクタは Editor のスレッドにも効いていた**（文脈 31,731 → 24,986 トークン・初回応答 4.2 → 1.8 秒）。
  ⑨ で Chat だけ止めたが、これは全スレッドの問題なので設定 `claude_ai_connectors`（既定 on = Claude Code の既定どおり）で
  切れるようにした。**Chat はこの設定に関わらず常に読み込まない**（「necoder の設定で選んだ物だけを渡す」原則を守る）。
