...
- **窓セッションは necoder.db に 1 窓 1 行（2026-09-03・本人要望「turso に寄せる・互換経路は捨てる」）** —
  旧 `state.json` は全窓共有の 1 ファイルを各窓が丸ごと上書きしており、別窓で閉じたタブが次回起動で
  復活する（最後に書いた窓が勝つ）事故の原因だった。窓ごとに行を分け、各窓は自分の行だけを更新、起動時は
  生存中の全窓を復元（VSCode の `restoreWindows: all` 相当）。ユーザーが閉じた窓は `closed_at` で復元対象から
  外すが、直近に閉じた 1 行だけは残し「全部閉じてから ⌘Q」でも最後の窓が戻る。⌘Q は閉じ印を付けず、
  その瞬間に開いていた窓以外の生存行を閉じ扱いにする（復元対象＝最後に終了したときの窓の集合）。
  行数は「生存中の窓 + 閉じ済み 1 行」で頭打ちなので古い行の自動消去は要らない。
  `state.json` の互換読み込みは持たない（残っているファイルは無視される）。終了直前には各窓の最新状態を
  DB へ同期保存し、background write を待たずにプロセスが終了して次回復元行が欠ける事故を防ぐ。
...
- **他ツールで見つけた MCP サーバは「一覧に載せるが既定 off」（2026-09-10）** —
  ACP では MCP サーバを渡すのがクライアントの仕事なので、necoder はどのサーバへ繋ぐかを自分で決める。
  ここで選択肢は 3 つあった: ①necoder の `settings.json` に書いたものだけを渡す（他ツールの登録は無視）
  ②`~/.codex/config.toml` 等で見つけたものを**そのまま全部渡す**（登録し直しが要らない）
  ③見つけたものは**一覧に載せるが、有効化は明示だけ**。
  採ったのは③。②は「他人（別ツール）の設定ファイルを根拠に、necoder が黙って子プロセスを起こす」
  ことになる。stdio サーバは任意のバイナリで、リモートサーバは繋いだ先で課金され得る。
  必要なのは**同意 1 回**であって、登録のやり直しではない。①は「Codex に入れたのに使えない」
  という今回の出発点をそのまま残すので採らない。
  necoder 自身の設定に書いたものは既定 on — そちらは書いた時点が同意だから。
  同じ理由で、`${VAR}` を展開できなかったサーバは**送らない**（`Bearer ${TOKEN}` という嘘の
  ヘッダで繋ぎに行かない）。渡せなかったものは transcript に 1 行出す（黙って落とすと
  「設定したのに動かない」がまた無言の失敗になる）。

- **通知音は録音ではなく合成で作る（2026-09-11）** —
  「完了音を猫の鳴き声にしたい」から始まった。まず CC0 の実録音（Wikimedia Commons）を拾って、
  ピッチを上げる・低域を削る・0.5 秒に詰めるところまでやったが、**どれも通知音にならなかった**。
  生録音は部屋鳴りとノイズ床があり、音程が無く、立ち上がりが鈍い。あれは「猫がそこにいる音」で
  あって「合図」ではない。通知音の条件は短い（〜0.5s）・音程がある・立ち上がりが鋭い、の 3 つ。
  採ったのは**加算合成**（`scripts/gen-chime.py`）。猫の鳴き声を似せるのではなく、猫の**節回し**
  （周波数が上がって下がる）だけを音程のある音に載せ、フォルマント 2 本で声に寄せる。得たものは 3 つ:
  ①権利が完全に自前（AGPL 配布で説明が要らない・`THIRD_PARTY_NOTICES.md` が増えない）
  ②後から調律できる（「高い」「長い」と思ったらスクリプトの数値を直して再生成する）
  ③40KB ×2 で済む。生き物の声を数式で似せるのは筋が悪いが、ベルと節回しの合成は得意な領域だった。
  ユーザーが自分の猫を録りたくなったときのために、設定値には**ファイルパスも書ける**ようにしてある。

- **Fleet v2: Task を主語にした 1 画面 + Captain（2026-09-12・本人承認。正は `FLEET-V2.md`）** —
  0.1.15 の作業タブ差し戻し後も「概念 8 つ（Task/セル/ペイン/面/列/管制/系譜/作業）・中央タブ 3 面・＋ボタン 3 つ」で
  「作る → 見張る → 裁く → 統合する」の流れが見えなかった。herdr / Orca / Conductor / cmux の比較
  （`research/fleet-ux-2026-09.md`）で GUI 勢が「左 = Task 一覧（状態つき）/ 中央 = 選んだ Task 1 つ / 並べるのは
  明示で 3 本まで」に収束していることを確認し、同じ型に畳む。確定 7 点:
  ① ユーザーに見せる概念は **Task / Task の中のタブ / Captain** の 3 つ。セルと面は Task カードとタブへ、作業タブと管制タブは廃止。
  ② **監督 → Captain** に改名し Fleet の中核に置く。人間の既定の玄関は Captain、Task への直接発話は「介入」として台帳に残し次の wake に同乗。
  Captain は状態を持たず（記憶は台帳）integrate と承認応答はできない（推薦のみ）。
  ③ ターミナルは Task の中のタブ。エージェントの起動は ACP が既定で CLI 形態は明示選択のみ（07-23 ⑥ のまま）。
  ④ 1 Task = 1 branch = 1 worktree（`git worktree add -b` で同時に作る）。同じ worktree に 2 体は「詳細」からのみ。
  ⑤ サイドバーはレールで選んだ 1 プロジェクトの編隊だけ。他プロジェクトはレールのドットと statusbar で足りる。
  ⑥ Rust のビルドコストは **`CARGO_TARGET_DIR` を統合先の `target/` に共有**（準備スクリプトが書く `task.env` を necoder が
  Task のプロセスに注入）を既定にする。依存は 1 回だけ・ディスクは増えない・代償は cargo のロックで同時ビルドが直列化すること。
  痛くなったら sccache（並列・ディスク 12GB × Task 数）。necoder 自身は言語の知識を持たない（スクリプト側の判断）。
  ⑦ 管制の全画面版はリモート管制（P9）の正として残す。実装は F0〜F7（各単独マージ可・`FLEET-V2.md` §10）。


- **control.necoder.com を本人運用の公開リレーにする（2026-09-13・本人）** —
  2026-07-25 の決定「既定のリレーは**本人が用意する**」「QR がそのまま資格情報＝**アカウント不要**」に実装を合わせた。
  v0.1.14 の実装は `POST /api/rooms` に単一の共有秘密 `PROVISION_TOKEN` を要求しており（**計画外の追加**）、
  結果として「各自が Cloudflare アカウントで Worker をデプロイする」形になっていた＝決定と逆で、配布時に誰も有効化できない。
  **撤去した**（Worker secret も削除）。使う側に要るのは Node.js 22+ と necoder だけ。
  **なぜ token 無しで安全か**: room id / host・phone token / E2E 長期鍵は**すべて Mac がローカル生成**し、
  リレーは token の SHA-256 ハッシュしか持たず、E2E 鍵は QR のフラグメント（`#`）にしか載らない＝
  リレーが認証すべき「身元」がそもそも存在しない。token は認証ではなく**部屋の量産を止めるゲート**でしかなかった。
  **代わりの濫用対策 3 枚**（ROADMAP P9c の「初日にレート制限/フレーム上限/room TTL/請求アラート」に対応）:
  ① Workers のレート制限 binding（`ROOM_LIMITER` 5回/60s/IP）。**colo ごとの近似で厳密な上限ではない**（実測: 10回/60s では 15 連打が素通り・5回/60s で 6 発目に落ちる）＝大量作成を鈍らせる一枚目
  ② 未ペア部屋は 300 秒で `alarm()` が `deleteAll()`（量産しても残らない）
  ③ `POW_DIFFICULTY`（既定 0＝無効）。上げると `/api/health` 経由でホストが自動追従するので、**ホストを配り直さずに**効かせられる。
  ペアリングは一生に数回なので 1〜2 秒の CPU は体感ゼロ、攻撃側には部屋あたり同じ費用がかかり IP 分散が効かなくなる。
  **未了**: Cloudflare Notifications の請求アラート（運用側の作業）。
  公開サービスとして走らせるので **AGPL §13** の対応ソース提供義務がかかるが、`source.tar.gz` の生成と PWA からのリンクで既に満たしている。
  配信元を乗っ取られると改変 JS で全ユーザーの端末内の鍵が危険になるため、Cloudflare アカウントの MFA と最小権限のデプロイ資格情報は必須とする。
