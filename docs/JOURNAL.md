...
- 次: ①手元を origin と合流（0.1.11 系）してから今回の変更を載せる ②metal のある環境で実機確認（前エントリの ①②③）③`assert_off_main_thread` を strict 既定へ倒す前に、残る UI スレッド I/O（`open_folder_in_rail` の `Worktree::with_host` + `TaskSpace::for_worktree`、`toggle_split`、`revert_hunk`、hot-exit 復元、`host_for_project`）を潰す
