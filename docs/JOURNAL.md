...
- 残: mac の `fleet_panes_keep_their_threads_and_remote_targets` が CI で時々落ちる（`Detected activity on thread Some("PTY reader") … Your test is not deterministic.`）。`add_terminal_to_selected_task` が**実シェルを起動する**ため、遅いランナーでは PTY のリーダースレッドが gpui のテストスケジューラに割り込む。3efcbef でも同じ落ち方をしており（その後 2 回は緑）、今回の削除とは無関係。直すなら「テストで実 PTY を作らない」側
- 次: M16 C0（変更なし）
