...
- やったこと: 約 13,000 行・約 95 直接フィールドだった `workspace.rs` を、開始 checkpoint `ca46f40` から小さい checkpoint で再編。`src/lib.rs` facade / persistence / `ProjectSource` / typed LSP parser・server registry を先に分離し、TerminalDock・SearchPanel・Explorer・GitPanel・TodoPanel・SettingsView を長寿命 Entity へ移した。編集面は `EditorArea`（tabs/pane、LSP/diagnostics/completion、navigation、diff/inline edit、hot exit）、project 単位の実行状態は `ProjectSession`（Agent/Terminal/Search/Explorer/Git/Todo、repository snapshot、watcher）、rail metadata と session 配列は非公開 `ProjectSessions` が所有する。`Workspace` は 8 直接フィールド、`workspace.rs` は 1,108 行。通常の project switch は pure helper で active index だけを変え、loaded session を再生成しない。command/panel 結線は `CommandRegistry` / `PanelRegistry`、`debug_*` は debug build のみに隔離。root Render に残っていた pending command 消化も effect-cycle 末尾へ移した。
- 設計判断: `EditorArea` 自身は `Entity` にせず、長寿命 aggregate の中に実体の `Entity<EditorView>` を保持する。二重 Entity 化は session 内の同期 editing command を不必要な cross-entity update にするため。Explorer/Git は state と typed event を独立 crate が所有し、window/chrome 操作を伴う描画 callback は shell adapter (`explorer_view.rs` / `git_view.rs`) に残した。これで child → workspace の逆依存を作らない。
- 検証: `cargo check --workspace` / `cargo test --workspace` / `cargo check -p shirushi --release` / `git diff --check`。構造監査で feature crate の workspace 非依存、`tabs.clear()` が初回/明示 reload のみ、全 `pub fn debug_*` の `cfg(debug_assertions)`、root Render の Host/FS/Git/DB 呼び出し無し、i18n ja/en parity を確認。設計・checkpoint・完了条件は [`REFACTOR-WORKSPACE.md`](./REFACTOR-WORKSPACE.md) に記録。
