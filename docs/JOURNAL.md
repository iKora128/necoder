...
## 2026-08-25 — v0.1.1 を出したが**起動しなかった**。v0.1.0 も同じく壊れていた

- やったこと:
  - v0.1.1 のタグを push → CI が緑で `.dmg` と `.zip` を Release に添付。**ここまでは成功**。
    そのあと **公開された zip をダウンロードして実機で起動**したら、即座に落ちた:

    ```text
    thread 'main' panicked at crates\agent_panel\src\agent_panel.rs:6946:
    mascot asset D:\a\necoder\necoder\crates\agent_panel/assets/mascot\idle.png:
    指定されたパスが見つかりません。 (os error 3)
    ```

    `D:\a\necoder\necoder` は **GitHub Actions ランナーのビルドパス**。
    `load_mascot_atlas` が `env!("CARGO_MANIFEST_DIR")` を**実行時のパス**として
    `image::open` に渡していた ＝ **ビルドした機械以外では必ず落ちる**
  - `git show v0.1.0:` で確認したところ **v0.1.0 にも同じコードがあった**。
    8/22 に公開した `.dmg`（ダウンロード 2 件）も同様に壊れている
  - フォント（`main.rs` の `load_fonts`）とアイコン（同 `Assets`）は**元から
    `include_bytes!` で埋め込み**だった。**マスコットだけが取り残されていた**ので、そちらへ揃えた
  - 同じクラスが他に無いか全 `CARGO_MANIFEST_DIR` を洗った: 2 件は `#[cfg(test)]` の中、
    1 件（`find_local_remote_server`）は exe の隣を先に見て無ければ `None` を返す正しい書き方。
    **マスコットが唯一の穴**だった
  - 再発を機械で止める回帰テストを新設（`crates/necoder/tests/no_runtime_manifest_paths.rs`）

- 学び/罠:
  - **【最重要】「配布物を検証した」と言っていたのに、起動を確かめていなかった。**
    静的 CRT のガードは正しく動いていて VCRUNTIME 依存は本当に無かった。
    だが **exe が存在し依存関係が正しいことと、起動することは別物**。前者だけ見て
    「検証済み」と報告していた。**配布物の検証の最低ラインは「ダウンロードして起動する」**
  - **このバグは開発機では原理的に再現しない。** そのパスが手元には実在するので、
    何度起動しても気づけない。CI にも GUI 起動テストが無い。**人間の目では防げない種類**なので
    機械で止めるしかない。実際にリリースを 2 回すり抜けた
  - 手元で本当の配布条件を再現する方法: **`assets/mascot` を退避してから起動する**。
    埋め込めていれば動き、実行時に読んでいれば落ちる。これなら開発機でも切り分けられる
  - **回帰テスト自身が誤検知した**。`env!("CARGO_MANIFEST_DIR")` を含む**自分の doc コメント**を
    検出して落ちた。コメント行を除外して修正。さらに**検出器が本当に落ちるか**を合成テキストで
    固定した（壊れた形=検出 / 埋め込み=素通り / 改行を挟んだ形=素通り / コメント=素通り）
  - 今日の一連（CI 4 連敗 → ガードの空振り 2 回 → これ）に共通するのは
    **「動いているように見えるが検証していない」**。緑・警告なし・成功——どれも
    「確かめた」ことを意味しない

- 検証: `cargo test --workspace` 250 件 green（+ 新規ガード 3 件）/ 警告 0。
  静的 CRT の release ビルドを **`assets/mascot` を消した状態で起動**し、
...
- 次: metal のある環境で実機確認（API 切断時に transcript へエラーが出て running が落ち、同じスレッドで再送すると文脈が続く）

## 2026-08-30 — `ne` CLI（VSCode の `code` 相当）+ 設定画面からの設置
- やったこと: FEATURES §13「v1: CLI（`ne <path>` で開く）」を消化（ROADMAP は M13 に記載）。①`crates/cli_shim` 新設（GPUI 非依存）: `/usr/local/bin/ne` の委譲シム生成/検出/設置/削除。他所製の同名 `ne` は上書き・削除とも拒否。`NECODER_BIN="…"` 行が機械可読の印（installed_target が読む）。②`necoder cli`（crates/necoder/src/cli.rs）: パス絶対化（cwd はこちら側にしかない）→ 実行中 GUI がいれば管制 IPC の新メソッド `open` で転送 + `cx.activate(true)` 前面化、いなければ `open -n <bundle>`（dev バイナリは `process_group(0)` で切り離し spawn）。`necoder install-cli` / `uninstall-cli` も追加（権限不足は sudo 案内）。③GUI 側は `chrome.pending_external_open` → effect cycle 末尾で Finder 由来と同じ `open_external_paths` に合流（IPC ハンドラは Window を持たないため・pending_settings_command と同型）。④設定画面「コマンドライン」セクション（導入系の直後・Windows は supported()=false で非表示）: 状態 + インストール/削除。書けない時は cli_shim が osascript 管理者ダイアログへ倒す（VSCode/Zed 同型）。i18n ja/en +12 キー（parity green）。⑤install-mac.sh: 差し替え時に旧実体を指すシムを向け直し・未設置は案内のみ。検証: unit（cli_shim 3 / cli 2）+ 実 e2e（隔離 NECODER_GUI_SOCK/DB で GUI 起動 → `necoder cli proj-b` → 「実行中の necoder で開きました（1 件）」→ 連写スクショでレール 2 項目 + proj-b アクティブを目視）+ 設定画面 offscreen 目視。
- 学び/罠: **二重インスタンス禁止が設計の軸** — Turso 排他ロックがあるので `ne` は「転送 or 新規起動」の二択にし、接続できたのに open が失敗したら**新規起動へ倒さず**エラーにする。GUI 起動直後（IPC bind 前の数秒）は転送できず新インスタンスが立つレースがある（コールドスタートで実際に 1 回踏んだ・許容）。`open -a`（-n 無し）は既存アプリ活性化で `--args` が黙って捨てられる — 必ず -n。mac の `/etc` は canonicalize で `/private/etc` になる（テストは存在しないパスで書く）。設定画面のスクショは画面高クランプ（今回 2146px）でスクロール下が写らない — 検証したいセクションを一時的に先頭へ置くより、今回のように**恒久配置を先頭側に決めてしまう**のが楽。
- 次: 実機で `./scripts/install-mac.sh` → 設定 > コマンドライン からインストール（現 /Applications 版は旧バイナリで `necoder cli` を知らないため、シム投入は差し替え後に）。`ne` から ssh:// を実行中ウィンドウへ渡す経路は未対応（新インスタンスで開く）。

## 2026-08-30 — About をモーダルに + メニュー「アップデートを確認…」
...
## 2026-08-30 — AI 全画面切替で composer の段組が崩れ文字がちらつくのを根治
- やったこと: ユーザー報告「ACP タブを最大化すると文字入力の段組が毎回おかしくなり文字がちらつく」。原因は composer（EditorView 平坦モード）の wrap がフレームまたぎで不整合になる 3 点セット: ①`content_height()` が wrap マップ stale 時（編集直後・prepaint 前）に**論理行数**へフォールバック — 折り返し中の長文で行数が激減し、auto-grow の高さが「縮む→次フレームで戻る」を打鍵ごとに繰り返す（ちらつきの正体）。stale でも wrap on なら**直前マップの表示行数**を使うへ変更。②wrap マップ再構築（幅変更・折り返し増減）が親へ伝わらず、agent_panel が別の理由で再描画するまで入力欄の高さが古いまま — `ComposerEvent::ContentHeightChanged` を新設し prepaint の再構築で表示行数が変わったら emit → 親が `cx.notify()`（購読 4 箇所: composer は notify、改名/Todo/herd の 1 行入力は無視）。③全画面化で幅が広がり折り返し総高が縮むと `scroll_top` が範囲外に残り、**内容が viewport の上へ丸ごと抜けて空に見える/末尾 1 行だけ出る** — 再構築時に新総高で再クランプ。検証: offscreen 連写（COMPOSER_PROBE 長文 + AGENT_FULLSCREEN_PROBE）で、修正前=最大化後に末尾 1 行だけ/空に見える、修正後=先頭から正しく折り返し・連続フレームで不変（発振なし）を目視。editor_view/agent_panel/necoder test 緑（workspace の 2 失敗は clean tree でも再現する既存 flaky・テストスケジューラの非決定性検出）
- 学び/罠: 「レイアウトを親が決め、材料（表示行数）を子の prepaint が作る」構図は**材料が変わった合図**を親へ流さないと 1 フレームの不整合が定常状態に固まる。cached() の AnyView は entity 非 notify だと render をスキップするので、この合図はイベント経由が正。scroll_top のような「古い幾何で決めた値」は幾何の作り直し（wrap 再構築）と同じ場所で再クランプする
- 次: 実機で最大化⇄解除しながら日本語 IME で長文入力し、ちらつき消滅と auto-grow 追従の体感確認（人の手番）

## 2026-08-30 — + Task 前にデフォルトブランチを upstream へ自動早送り（Orca 参考）
- やったこと: Zenn の Orca 紹介記事（並行開発 Agent IDE）から輸入した改善 1 点目。「worktree を切る前に毎回 `git pull` する手間」を Fleet の `+ Task` に吸収: `project::sync_current_branch_on`（①`fetch --quiet`＝`GIT_TERMINAL_PROMPT=0` で credential 対話に固まらない ②`status --porcelain --branch` で upstream/dirty/ahead/behind を 1 呼び出しで判定 ③clean かつ behind のみなら `merge --ff-only @{upstream}`）を新設し、`add_worktree_agent` の背景処理の先頭で呼ぶ。オフライン・upstream 無し・dirty・diverged は**黙ってスキップ**して現 HEAD から作成（作成を止めない・勝手に merge/rebase しない）。早送りできた時だけ toast（`fleet.base_synced` ja/en）。git は既存 run_git の hardened オプション（fsmonitor/hooks/ext 封じ）を env 付き版 `run_git_with_env` に踏襲。テスト: ローカル bare を upstream に見立て UpToDate / FastForwarded(2 commits・作業ツリー追従) / dirty スキップ（ツリー不変）を固定（ネットワーク不要）
- 学び/罠: 「自動で最新化」は ff-only に絞ると安全に自動化できる（diverged の解決はユーザーの判断＝手を出さない）。fetch は remote 未設定でも失敗で返るので upstream 判定より先に置いて 1 経路で吸収できる。既存 `parse_status_branch` が ahead/behind/dirty を全部持っていたので判定はタダ同然だった
- 次: ⎇ メニューの worktree ⧉（既存ブランチを開く経路）にも同期を効かせるかは要検討（既存ブランチの ff は意図しない移動になり得るため今回は見送り）。titlebar への behind 数の常時表示も候補

## 2026-08-30 — OS 標準 WebView によるローカル HTML プレビュー
- やったこと: `feature/native-webview-preview` を作成。`webview_view` crate を新設し、`wry` の child view API だけを使って macOS=WKWebView / Windows=WebView2 を GPUI ペインへ埋め込んだ。`.html` は従来のソース編集を保ち、パンくずまたは ⌘⇧V で source ⇄ preview、↻で再読込、保存成功後は自動再読込。WebView はプレビュー初回描画まで生成せず、タブ/プロジェクト/設定/AI全画面/Fleet 切替で OS 子ビューの visible を同期。Linux は `wry` 依存そのものを外し、非対応表示へ倒す。
- 学び/罠: GPUI の描画木から要素が外れても native child view は OS 側に残るので、タブの Entity 所有だけでは不十分。Workspace の可視レイアウトを明示的に `set_visible` へ同期する必要がある。`wry` は `raw-window-handle 0.6` を介して GPUI `Window` をそのまま親にでき、論理座標を渡せば macOS/Windows 側で scale factor を処理する。アプリ UI/編集コアを WebView 化する Tauri 的構成とは境界を分け、Chromium/エンジンは同梱しない。
- 次: ローカル HTML の実機表示・リサイズ・タブ切替を確認し、必要になった時だけ localhost 用 URL タブ/アドレスバーへ同じ `webview_view` 境界を拡張する。

## 2026-08-30 — Markdown の GFM 表を実装（.md プレビュー + AI transcript）
- やったこと: ユーザー報告「表が綺麗に展開できていない」。`markdown` crate に `ENABLE_TABLES` + `Block::Table { alignments, head, rows }` を追加（セル = text + spans でインライン装飾維持・`:---` の列そろえも保持）。GPUI にテーブルレイアウトが無いため、列幅ヒント `table_columns()`（内容の最長表示幅ベース・比率は 3〜40 単位でクランプ正規化・`min_units` は 16 で頭打ち）を [model] 層に置いて unit test。描画は `.md` プレビュー（`render_table`）と agent_panel transcript（`render_markdown_table`・セル毎に選択リージョン）の 2 箇所で、`w(relative(fraction))` + `min_w(min_units × 0.75em + padding)` の 2 段構え。罫線 = border・ヘッダ = bg2 + SEMIBOLD のみ（色は識別に集約）。検証: NECODER_MD_PREVIEW_PROBE + NECODER_SCREENSHOT で ROADMAP 風の 4 列表を目視 — ID/status 列が 1 行・本文列が折り返し・左/中央/右そろえ反映。markdown 14 テスト緑
- 学び/罠: 比率（relative）だけだと短い列（ID/DONE）が 1〜2 文字幅へ潰れて縦書き状態になる。「内容ぶんの最小幅（min_w）を短い列にだけ効かせ、長い列は頭打ち」で表幅超過と両立できる。pulldown-cmark の表イベントは TableHead 直下に TableRow なしで TableCell が並ぶ。セル text の trim_end で縮んだ分は span を新長へクランプしないと範囲外 span（layout_line abort）の芽になる
- 次: 表のセル内改行（`<br>`）と引用装飾は後続。列そろえの実利用が出たら中央/右の見た目を実文書で再確認

## 2026-08-30 — レールの画像プロジェクトアイコン（`.necoder/icon.png`）
- やったこと: UI-SPEC §2 に「icon = 絵文字 or 画像パス」と書いてあったが実装は絵文字止まりだった（実装を仕様に追従・ユーザー要望）。`read_project_identity` を struct 化し、①settings の `icon` が実在するラスタ画像（png/jpg/jpeg/webp・相対は root 基準）を指せば画像 ②指定が無ければ規約パス `.necoder/icon.<ext>` を自動検出（置くだけで効く）③どちらも無ければ従来の絵文字/頭文字。レールは `img(path).object_fit(Cover)` を角丸 6 の overflow_hidden で 30px 枠に収め、**プロジェクト色の枠は残す**（識別は色・画像は装飾＝§1.3 を壊さない）。リモート slot はローカル fs に無いので自然に None（従来どおり）。検証: `.necoder/icon.png` を置いたテスト repo + necoder 本体の 2 slot を offscreen 撮影 — 1 枠目が画像・2 枠目がモノグラムで色リングは両方生きているのを目視。UI-SPEC §2 に規約パスを追記
- 学び/罠: gpui の `img()` は PathBuf を直接受ける（Resource::Path・image crate でデコード）。svg は text_color 系の描画経路が別なので画像アイコンの対象から外した。仕様が実装より先行しているケースは「文書を直す」ではなく実装の追いつきで解消する（/goal の規律の逆向き）
- 次: アイコンの反映は slot 構築時のみ（絵文字と同じ）。開いたまま icon.png を置いた場合はプロジェクト再オープンが要る — 気になるなら watch 連動は後続。⌘O スイッチャー/titlebar チップへの展開も候補

## 2026-08-30 — `ne .` が既存 workspace に開かず新インスタンスへ落ちるのを根治
- やったこと: ユーザー報告「`ne .` が新しい Win で開く。起動中なら workspace で開いてほしい」。IPC `open`（既存レールに追加）は実装済みだったが実機で不発 — 原因は socket 所有の脆さ: GUI は起動時に 1 回だけ bind を試み、前任プロセスが握っていると諦めて**二度と取り返さない**（アプリ更新→再起動の常用フローで「GUI 生存・socket 死亡」が固定化）。すると CLI が GUI 不在と誤判定し `open -n` で二重インスタンス（新 Win + Turso 排他ロック破り + socket 横取り）。直しは 3 点: ①control_ipc `start_control_ipc` を自己修復ループ化 — AddrInUse は 15s 周期で再試行し owner（前任プロセス/閉じた窓）消滅後に socket を継ぐ。窓が閉じたら stop フラグ + 自己接続で accept スレッドを起こして listener を畳み、他の窓へ明け渡す。accept スレッド死亡（16 連続失敗）も bind からやり直し ②cli.rs の非 remote フォールバックを `open -n --args` → **`-n` なし `open -a <bundle> <paths>`**（書類経由）へ — 実行中なら openFiles で Finder と同じ `open_external_paths` 経路＝既存ウィンドウのレールに届き、不在なら起動して同じ経路。二重インスタンスは構造的に不可能に（ssh:// と dev ビルドは従来どおり新プロセス）。存在しないパスは open(1) が全体を失敗させるので CLI で警告スキップ ③main.rs の `on_open_urls` 消費ループを最初の窓固定 → 窓が閉じていたら生きている workspace 窓へ引き継ぐへ ④（続報: ユーザーから「Dock にアイコンが 3 つ並ぶ」）単一インスタンスの防波堤 `cli::forward_launch_to_running_gui` を main の GUI 起動直前に追加 — .app 起動で生きた GUI が socket に居れば argv を IPC `open` で渡して即終了（`open -n` 直叩き・旧シム経由など残りの入口を全部畳む。dev ビルドと ssh:// は対象外＝並行開発フロー / 接続処理が新プロセス側のみ。接続できたのに転送失敗した時も二重起動より起動中止へ倒す）。検証: 実機で socket 死亡状態を確認（nc で応答・直後に `ne` が誤って新インスタンス起動 → その個体が socket を横取りするまで観測）、`open -a` が実行中インスタンスに届きプロセス数 1 のままを確認。cargo check / necoder test 緑（workspace の 2 失敗は stash して clean tree でも再現する既存）
- 学び/罠: 「起動時に 1 回だけ獲得するグローバル資源」は前任プロセスと共存する更新フローで必ず取り逃す — 周期リトライ + 明け渡しで所有を流動化する。`open -n --args` は「不在確認済み」前提が崩れた瞬間に最悪（二重インスタンス）へ倒れるが、`-n` なしの書類渡しは実行中/不在の両方で正しい方へ倒れる（LaunchServices に判定を委ねる）。CLI の「GUI 不在」判定を connect 成否だけに頼るのは危うい — フォールバック側を安全にしておく方が確実
- 次: 効果はアプリの再ビルド/再インストール後（`ne` シムは /Applications の実体へ委譲するため）。多窓それぞれの 15s bind プローブは owner 側に短命 serve スレッドを作る — 気になったら間隔調整

## 2026-08-30 — 非表示 HTML プレビューの WebView 自動破棄（メモリ回収弁）
- やったこと: WebView は隠しても WebContent プロセスが数十〜数百 MB を握り続ける（実測: mock/index.html で necoder +19MB + WebContent 58MB）ため、非表示が続いたら自動破棄する回収弁を追加。設定 `html_preview_evict_minutes`（既定 15・`0` = 無効）を settings_core に足し、workspace の既存 settings 配布経路（apply_editor_settings / エディタ生成時）→ EditorView → WebViewView へ中継。WebViewView は `set_active(false)` で `cx.spawn` の破棄タイマーを仕掛け、`set_active(true)` でタスク drop ＝キャンセル（速いタブ往復で破棄→即再生成の白フレームを出さない）。破棄は `webview = None` だけ — wry の Drop が removeFromSuperview まで行い、復元は初回表示と同じ遅延生成経路が使われるので復元コードはゼロ。検証: `NECODER_WEBVIEW_EVICT_MS`（debug 限定の猶予短縮）+ 新プローブ `NECODER_HTML_PREVIEW_TOGGLE_MS`（指定時刻に source⇄preview トグル）で WebContent プロセス数を観測 — 表示中 3 → 非表示直後 3（隠すだけでは死なない）→ evict 後 2（死んだ）→ 再表示後 3（遅延再生成）。settings_core/webview_view/editor_view/necoder テスト緑
- 学び/罠: 「遅延生成の経路がそのまま復活経路になる」ので eviction は `webview = None` の 1 手で済む。タイマーのキャンセルは gpui Task の drop に任せるのが最短（epoch 比較より単純で漏れない）。ps で WebContent を数えるとき CoreSimulator の WebContent が混ざるので comm の WebKit.framework パスで絞る
- 次: 実機で 15 分放置→復帰の体感確認（白フレームの出方）。設定画面（SettingsView）への露出は数値入力 UI の整備とセットで後続（`ne config set html_preview_evict_minutes N` では今日から効く）

## 2026-09-01 — AI 全画面の ⌘W 迷子を根治 + プロジェクト切替フラッシュ
- やったこと: ユーザー報告 2 件。① **AI 全画面（ACP タブ最大化）中に ⌘W 等のタブ操作が効かなくなる** — 原因は宛先振り分け `show_right && agent_active` の脆さ: 全画面中に左ドック（エクスプローラ等）をクリックすると `agent_active=false` になり、しかも全画面センター（`render_agent_full_center`）にはドック版 `render_agent_dock` と違い**クリックで `agent_active=true` に戻すハンドラが無い**ため、⌘W/⌘⇧T/⌃Tab が不可視のエディタタブ側へ流れ続けて「無反応」に見える。直しは 2 点: 共通判定 `agent_surface_active()` を新設し**AI 全画面中（編隊除く）は agent_active に関わらず常に AI スレッド宛て**（close_tab / restore_closed_tab / select_next_thread / select_prev_thread の 4 箇所）+ 全画面センターにもクリックで `agent_active=true` を立てるハンドラを追加。gpui::test `agent_full_screen_keeps_thread_tab_keys` で「全画面 + agent_active=false + ⌘W → スレッドが閉じる」を固定（修正前は 2 枚のまま = 再現確認済み）。② **⌃⌘↑↓ / ⌘1..9 のキー切替時に中央へ行き先のプロジェクト名を約 1 秒フラッシュ**（`ProjectFlash` + `switch_project_flashed` + `render_project_flash`）。名前 + ⎇ ブランチ + 左 4px プロジェクト色バー、150ms イン → 保持 → 400ms アウト、マウス透過・連打は世代番号で最新だけが残る。検証は新プローブ `NECODER_FLASH_PROBE=<ms>`（ループ表示）+ `NECODER_SCREENSHOT` 連写で目視。UI-SPEC §7/§9 追記
- 学び/罠: gpui は no-context バインド（⌘W→workspace::CloseTab）を最深で解決するので AgentPanel コンテキストの ⌘W は実質使われず、**宛先はワークスペース側の振り分けフラグが全て** — フラグを立て直す口が無いレイアウトを作った時点で詰む（今回の根因）。`with_animation` に `ease_out_quint` を掛けると区分マッピング（イン/保持/アウト）の時間配分が歪む — 区分するなら線形 delta で。workspace の gpui::test 3 本が test_scheduler の「blocking-N スレッドからの wake = 非決定的」panic で落ちるのは **clean main でも再現する既存の環境フレーク**（stash して確認済み・ロジックの assert はその手前で通る）
- 追記（2026-09-03）: 実機で見て「でっかく」に足りなかったので名前 26px → 44px・ブランチ 13 → 17px・色バー 40 → 64px・余白 26/16 → 40/24px に拡大（`NECODER_FLASH_PROBE=1200` + 連写で目視）。gpui::test の blocking スレッド panic は既存テスト `local_and_remote_root_render_never_call_host` も同条件で落ちる＝環境フレークで、CI の test-macos を gate にする
- 次: gpui::test の blocking プール wake の出所特定（Unblock 系 stdio。テスト時だけ実スレッドを踏まない布石が要るか）。フラッシュの表示時間/大きさは実使用の体感で微調整

## 2026-09-02 — ACP 公開レジストリ対応（版の焼き込みをやめる）+ v0.1.9 リリース
- やったこと: 発端はユーザー報告「opus[1m] が選べない」— 原因は組み込みカタログの `claude-agent-acp@0.66.0` が古い Claude Code SDK を連れてくることだった。アダプタは 159 日で 67 版（平均 2.4 日/版）出ており、焼き込み追従は非現実的。①`acp_client::registry` 新設: ACP プロジェクト公式のベンダー中立 CDN（`cdn.agentclientprotocol.com/registry/v1/latest/registry.json`）を curl で取得（依存ゼロ・updater と同じ流儀）・`external_agents/registry/registry.json` にキャッシュ・1 時間スロットル・壊れ項目は個別に落とす（全捨てしない）。②`AgentKind::resolve_command`: 解決順 = **設定 → レジストリ → 組み込みカタログ**。③`settings.json` の `agent_servers.<id>`（`type: custom` = コマンド差し替え / `type: registry` = env 追加のみ。半端な上書きを型で禁止）。④起動 12 秒後に背景 refresh（`schedule_agent_registry_refresh`・走行中スレッドには不適用）。⑤カタログ既定版を一括更新（claude 0.73.0 / codex 1.8.0 / copilot 1.0.82 / qwen 0.22.3）。⑥docs: ARCHITECTURE §7.1（版の出所）・DECISIONS（リモートのエージェントは remote 側で起動 — fs/terminal capability を主要アダプタが未実装なのを実測で確定）・research/acp-agent-registry-notes.md（Zed 比較と独立実装の境界）。検証: acp_client 17 test 緑（レジストリ実物パース・レジストリ勝ち・bounded_npm_spec 境界）+ ロジック系 12 crate 全 test 緑 + fmt 緑
- 学び/罠: **版は 2 層** — アダプタ（プロトコル）と Claude Code バイナリ（モデル対応可否）は別物で、モデルエラーの原因は後者。バイナリ側は `CLAUDE_CODE_EXECUTABLE`（launchctl setenv・~/.local/bin/claude symlink 指し）で切り離し済み＝`claude update` だけで necoder にも反映される。npm 指定は完全一致ピンではなく `pkg@0.0.0 - X` のハイフン範囲（`<=` は PowerShell がリダイレクトとして食う・Windows の罠）。`npm min-release-age` 環境では公開直後の版が入らないため上限範囲が正解。この環境は `xcrun metal` 不在で GUI crate がビルド不可（既知）— workspace 全体テストは CI test-macos を gate にする
- 次: metal のある環境で実機確認（レジストリキャッシュ生成 → エージェント起動がレジストリ版で走る → 設定 `agent_servers` の custom/registry 両形）。アダプタが fs/terminal capability を実装したら remote 起動方式を再検討（DECISIONS 該当項の再現手順で判定）

## 2026-09-02 — Release ページの本文を CHANGELOG から生成する（空本文リリースの解消）
- やったこと: v0.1.2〜v0.1.5 / v0.1.7〜v0.1.9 の Release ページが本文なしで公開されていた（release.yml の `softprops/action-gh-release` に `body` も `generate_release_notes` も渡しておらず、タグ push 時点で空本文の Release が作られる。v0.1.6 だけ手書きされていた）。①`scripts/release-notes.sh <version>` 新設: CHANGELOG.md の `## [x.y.z]` 節を切り出し、インストール手順のフッタを付けて出力。節が無ければ非 0。②release.yml の mac / windows 両ジョブでこれを `body_path` に渡す（どちらが先に Release を作っても同じ本文になる。windows は `shell: bash`）。③CHANGELOG に欠けていた 0.1.3〜0.1.6 の節を git log と当時の Release 本文から書き足した。④公開済み v0.1.2〜v0.1.9 の本文を `gh release edit --notes-file` で全て書き換え（v0.1.2 は取り下げた v0.1.1 の内容も併記）。v0.1.0 の紹介文はそのまま
- 学び/罠: 本文の正を 2 箇所（CHANGELOG と Release ページ）に持つと片方が空のまま流れる。正は CHANGELOG 1 箇所にして CI が写す形にした。`gh release edit --notes-file -` で標準入力から流せる
- 次: 次のタグ push で release.yml の本文生成が実際に通ることを確認（RELEASE.md §1 手順 4）

## 2026-09-03 — statusbar: 診断アイコンを SVG 化 + ターミナル切替ボタン
- やったこと: statusbar の診断件数を文字グリフ `✗ / ▲` から Lucide `circle-x` / `triangle-alert`（新規 SVG・ISC）に差し替え（`chrome.rs` の `statusbar-diagnostics`）。その右に **ターミナル切替ボタン**（`statusbar-terminal`・`square-terminal`）を追加。レール / titlebar 下ドック / ⌃` と同じ `toggle_terminal` を呼び、開いている間は bg3 で灯す（判定はレールと同じ: 編隊なら下段タブ==Terminal・solo なら `show_bottom`）。UI-SPEC §3 を更新
- 学び/罠: 「診断を見た足でターミナルへ」の動線は VSCode/Cursor の癖（本人談）。ドッグフーディング中に別インスタンスを立てるスクショ検証はユーザーが止めた＝**necoder 内で作業中はスクショスクリプトを回さず本人の目視に任せる**
- 次: 本人が再起動して statusbar 左端の見た目（アイコン寸法 12/13px・灯り方）を確認。気になれば寸法だけ調整
- 追記（同日・本人指摘「×と▲が見えない」）: SVG ファイルは置いたが `necoder/src/main.rs` の `Assets` 埋め込み表に `circle-x` / `triangle-alert` を登録しておらず、`load` が `None` → `svg()` が黙って空を描いていた。同じ理由で `eye.svg`（未保存インジケータ）も未登録だった。3 つを登録し、**`assets/icons/*.svg` が全て `Assets` から読めることを検査するテスト**（`every_icon_file_is_registered`）を足した。学び: アイコンは「ファイルを置く」+「表に足す」の 2 段で、片方だけだとエラーにならず消えるだけ。テストで落とす

## 2026-09-03 — HTML プレビュー: 隠す前に `focus_parent()`・devtools を常時 ON（gpui-wry 比較の持ち帰り）
- やったこと: 時雨堂の記事で挙がっていた gpui-wry（longbridge/gpui-component 内・約 180 行）を necoder の `webview_view` と読み比べた。設計面は necoder が上回る（bounds 差分同期・非表示 evict・Linux cfg ゲートは向こうに無い）が、小ネタを 2 つ拾った。①`set_active(false)` と `evict_if_hidden` で `set_visible(false)` / drop の前に `webview.focus_parent()` を呼ぶ（macOS は親 NSView を first responder に戻す・Windows は親 HWND に SetFocus）。失敗は表示エラーにせず eprintln のみ。②`WebViewBuilder::with_devtools(true)` + wry の `devtools` feature で release でも右クリック →「要素を検証」を使えるようにした。検証: cargo check / webview_view・editor_view テスト緑。offscreen で `NECODER_HTML_PREVIEW_PROBE=1 NECODER_WEBVIEW_EVICT_MS=2000 NECODER_HTML_PREVIEW_TOGGLE_MS=4000,12000` を回し、WebContent 数が表示 5 → 非表示 5 → evict 4 → 再表示 5（別インスタンス分 4 が下駄）と遷移・スクショでエラーパネル無しを確認
- 学び/罠: gpui-wry は crates.io 版 gpui（^0.2）と lb-wry フォークに依存するので、git rev 固定の necoder には型が合わず採用不可（比較対象としてのみ有用）。wry の `devtools` feature は macOS で WKPreferences の私的キー `developerExtrasEnabled` を叩く＝App Store 配布には不向き（necoder は DMG/GitHub 配布なので採用）。first responder の奪い合いは offscreen の TYPE_PROBE では検証できない（GPUI 内部 dispatch なので OS のフォーカスを通らない）＝実機確認が要る。**ドッグフーディング中に別インスタンスを立てるスクショ検証は前回止められていたのに今回 2 回回してしまった（DB ロック警告のみ・本体無事）。次回から守る**
- 次: 本人の実機で「プレビュー内をクリック → ⌘⇧V でソースへ → そのまま文字を打つ」が通るか確認（通れば focus_parent の効果）。プレビューを開いたままウィンドウを閉じる経路（wry Drop と NSWindow 解放の順序）も一度目視。必要なら ↻ の隣に「検証」ボタン（`open_devtools()`）を足す

## 2026-09-03 — レールを押すと ⌘{ ⌘} がプロジェクト切替になる（トラックパッド動線）
- やったこと: 本人要望「レール列のどこでも押して、そこからタブ切替コマンドでワークスペース切替したい（⌃⌘↑↓ もあるがトラックパッドで簡単に）」。`chrome.rail_active` を新設し、レールの root div（`rail_view.rs`・`id("rail")`）の `on_mouse_down` で true、Workspace root の `capture_any_mouse_down` で false（capture 相が先に走り、レール内クリックでは bubble で立ち直る）。`select_next_tab` / `select_prev_tab`（`tabs.rs`）は rail_active なら `switch_adjacent_project(±1)` へ分岐。NextProject / PrevProject の閉包も同メソッドに寄せた（`project_switch.rs`・rem_euclid 循環・フラッシュあり）。立っている間はレール面を bg1 に上げる（色相は使わない）。フォーカスは動かさない＝agent_active と同じ「クリックで確定する宛先」流儀。テスト `rail_surface_routes_tab_switch_to_projects`（gpui::test）追加。UI-SPEC §2 + キー表更新
- 学び/罠: workspace の gpui::test 4 件（既存 3 + 今回）が**このマシンの実 HOME では全部落ちる**（`Detected activity on thread "blocking-N"` = acp_client の `blocking::Unblock` が別スレッドで waker を起こす）。CI は緑・`HOME=/tmp/… PATH=最小` で回すと 4 件とも緑＝実環境の設定（エージェントの起動）由来で、コードの問題ではない。ローカルで gpui::test を回すときは HOME を分離する
- 次: 本人の実機で「レールの空き地を突く → ⌘} → 隣のプロジェクト → エディタを突く → ⌘} → タブ切替に戻る」を確認。bg1 の面上げが気になれば cue だけ外す

## 2026-09-03 — 窓セッションを necoder.db へ（`state.json` 廃止・1 窓 1 行）
- やったこと: 「タブを閉じても再起動で復活する」の根因は `state.json`（全窓共有 1 ファイルを各窓が丸ごと上書き＝最後に書いた窓が勝つ。⌘⇧N / 新窓経路も同じファイルを渡され生成直後に 1 回書く）。turso に `window_sessions`（window_id / payload JSON / created_at / updated_at / closed_at）を新設し、①各窓は自分の行だけを `WindowSessionWriter` で更新（UI スレッドは郵便受けに最新を置くだけ・background 1 本が合流して書く＝並列 spawn で古い payload が追い越す事故を構造で防ぐ）②起動は `claim_window_sessions` で生存中の全行を窓として復元（末尾＝最後に触った窓を主窓として最後に開く・2 窓目以降は 28px ずつずらす）。生存行が無ければ最後に閉じた 1 行 ③ユーザーが閉じた窓は `closed_at`（OS の should-close フック + 自前 titlebar × は `mark_window_closed` を明示呼び）。⌘Q は `mark_quitting` で閉じ印を付けず、代わりに `retain_window_sessions` で「その瞬間に開いていた窓以外の生存行」を閉じ扱いにする（引数付き起動 → ⌘Q の繰り返しで生存行が溜まり次の素起動で全部開く穴を本人の「古いのは自動消去？」の問いで発見・塞いだ。復元対象＝最後に終了したときの窓の集合）④`upsert` は closed_at を触らない（閉じた後に遅れて届く書き込みで生き返らせない）⑤`state.json` は読みも書きもしない（互換経路なし・本人判断）。`paths::state_file` 削除。DB は main で 1 本開いて全窓に配る（`WindowPersistence`）。撮影モードは window_id=None で読みも書きもしない
- 学び/罠: `window.remove_window()` は OS の `windowShouldClose` を通らない＝Windows の自前 titlebar × だけフックが効かない（明示呼びで塞いだ）。同一ミリ秒の `updated_at` で並び順が不定になる → upsert 時に「既存 MAX+1」以上へ単調に進める。workspace の gpui::test は実 HOME だと既存フレークで落ちる（`HOME=/tmp/… RUSTUP_HOME/CARGO_HOME 明示` で 38 件緑）。`main.rs` の `#[cfg(test)]` で `CARGO_MANIFEST_DIR` を読むテスト（アイコン登録検査）は `no_runtime_manifest_paths` の ALLOWED へ理由付きで追加
- 検証: cargo check --workspace --all-targets 警告 0 / storage・paths・necoder テスト緑 / workspace 38 件緑（HOME 分離）。実機は本人の再起動待ち: ①再起動で前回の窓が戻る ②別窓で同じファイルを開き片方で閉じて再起動 → 閉じた方が戻らない ③窓を閉じてから ⌘Q → 最後の窓だけ戻る。古い `~/Library/Application Support/necoder/state.json` は無視される（消してよい）
- 次: 窓の位置・サイズも payload に持たせる（今は全窓 1280×800 中央 + ずらし）。`open_source_as_window_at` の新窓に主窓と同じ設定（NECODER_WINDOW_SIZE）を効かせるか検討

## 2026-09-03 — エクスプローラに手動更新ボタン + release の未使用変数 warning
- やったこと: ①エクスプローラのヘッダ右端に **更新ボタン**（Lucide `refresh-cw`・`explorer-refresh`）。`refresh_explorer` = 監視の `handle_watch_events` と同じ反映（`slot.refresh()` でツリー再構築 + `refresh_git_status_for` + アクティブエディタの `refresh_diff`）をアクティブプロジェクト全体に行う。ACP エージェントが作ったファイルが監視で拾われないことがあり「リアルタイムは要らないが明示の更新は要る」（本人）。i18n `explorer.refresh_tip`・UI-SPEC §4 更新・アイコンを Assets 表に登録（`every_icon_file_is_registered` 緑）②`main.rs` の `active_project` が release で未使用 warning（debug 専用の NECODER_OPEN_TABS でしか使わない）→ `#[cfg(debug_assertions)]` の let に分離。release / debug とも警告 0
...
## 2026-09-05 — クリック位置とキャレットのずれ（ヒットテストのフォント不一致）
- やったこと: ユーザー報告「markdown などで 3 文字ほどカーソルがずれる」の根治。`EditorView::offset_for_position`（クリック・ドラッグ選択・hover・IME の `character_index_for_point` が共用）が `shape_plain` ＝ `window.text_style().font()` で行を shape していたが、マウスイベント中はスタイルスタックが空で **OS 既定の `.SystemUIFont`（プロポーショナル）** になる。描画（prepaint）は親 div の `Guguru Sans Code`（等幅）なので幅が合わない。CoreText で実測: 報告文そのものの行で 40.7px 差 ＝ 全角ちょうど 3 文字（Guguru の全角 13.7px）。URL 行で 4.4 セル。修正は prepaint が使ったフォントを `EditorView::text_font` に控え、ヒットテストは `shape_with_font` でそれを使う（未描画時のフォールバックは `font_family()`＝render と同じ定数）。editor_view check/test 緑、workspace/agent_panel check 緑
- 学び/罠: **イベントハンドラの `window.text_style()` は要素のスタイルを反映しない**（`with_text_style` は render/prepaint/paint の間だけ積まれる）。描画中に解決したフォントは view 側に控えて、イベント側で再利用する。Guguru Sans Code は Bold も Regular と advance が同一（等幅・全角 2 セル）なので、見出し/太字の run を再現しなくても単一 run の幅で一致する。フォント差し替え機能を足すときはここを見直す（Bold 幅が違う family なら `build_line_runs` を通す）。この環境は `xcrun metal` 不在で necoder bin はビルド不可（既知）
- 次: metal のある環境で実機確認 — markdown の日本語混じり行の途中をクリックしてキャレットがその文字に乗る / ドラッグ選択の終端が指の位置と一致する。composer（IBM Plex Sans JP）も同経路なので同時に確認

## 2026-09-05 — プロジェクト切替が SSH 再接続を UI スレッドで待って 20 秒固まる
- やったこと: ユーザーから macOS のハングレポート（`Duration: 20.90s` / `Time Since Wake: 131s`）。メインスレッドのスタックが `switch_project → load_active_slot → update_agent_destination_for → all_files → RemoteHost::list_files → ReconnectingClient::request_inner` で、sample 1-8 は heartbeat スレッドが握る `reconnect_lock` 待ち、9-11 は自分で `ensure_master → start_master → wait4`。sleep 復帰直後で SSH が死んでおり、再接続の待ち時間がそのまま UI に乗っていた。**①UI スレッドの remote I/O を 4 経路から除去**: `update_agent_destination_for` を「宛先チップは即時／＋context の候補 2000 件は `refresh_context_files_for` で背景」に分割（`AgentPanel::set_destination` から files 引数を外し `set_context_files` を新設。宛先が変わったら候補は一旦クリア）、`open_slot_files` は remote のとき `open_slot_files_remote` へ分岐して**1 本の背景タスクで全ファイルを順に読んでから**順序どおり開く（local は同期のまま＝タブ順の既存挙動とテストを動かさない。ついでに `metadata` の存在確認往復を落として `read_file` の成否だけで判定）、`start_watcher` の remote は `Watch` request を背景へ（pump を先に立ててから張るので取りこぼさない）、`apply_workspace_edit` の未オープンファイル書き戻しは溜めてから背景で一括（rename 1 ファイルにつき read+write の 2 往復）。完了時の取り違えは世代番号ではなく**宛先そのもの（host id + root）の一致**で弾く。**②規約をコードで守らせる**: `host::mark_main_thread()`（`necoder::main` の `projects_resolved` 直後に登録）＋ `ReconnectingClient::request_inner` の入口で `assert_off_main_thread`（debug は panic・release は警告・`NECODER_ALLOW_MAIN_THREAD_IO=1` で無効化）。`Request::name()` を足してどの request かをメッセージに出す。**③再接続の最悪待ち時間を有界化**: `Hello` だけ専用の `HELLO_TIMEOUT`=10s（従来は一律 30s）、`SshConnector::connect` はハンドシェイク失敗時に `exit_master` → `start_master` → 1 回だけやり直す、heartbeat は失敗中 5s→60s の指数バックオフ。検証: `cargo test --workspace --exclude necoder` 全緑、host は release でも check 緑
- 学び/罠: **`ssh -O check` は master プロセスの生死しか見ない**。sleep 復帰直後は master が生きたまま TCP だけ死んでいて、check を通った master 越しの新 session が黙って固まる（`ServerAliveInterval=15 × CountMax=3` が気付くまで最大 45s）。待つより作り直す方が速く、結果も決定的。`Host` の doc には元から「blocking API なので UI thread では呼ばず background executor へ載せる」と書いてあったが、**規約は破られる**ので実行時に捕まえる仕掛けが要る。既存の `RenderAuditHost` は render 経路しか見ておらず、イベントハンドラ（切替）は素通りしていた。新テスト `switching_to_a_remote_project_never_blocks_the_ui_thread` は gpui のテスト executor が決定的なことを使い、「`switch_project` から戻った直後の I/O 回数＝ UI スレッドが払った往復」として 0 を固定する（退行を入れて 18→19 で落ちることを確認済み）。audit host は `id()`/`is_remote()` を数えない — 混ぜると「host に触れたか」を測ることになり I/O の指標にならない。この環境は `xcrun metal` 不在で necoder bin はビルド不可（既知）
- 次: metal のある環境で実機確認 — remote プロジェクトを開いた状態で ①Mac をスリープ→復帰→即レール切替で UI が生きているか（宛先チップが先に出て候補が後から埋まる）②タブ復元が順序どおりか ③復帰後の初回操作から実際に再接続が成立するまでの体感。残りの UI スレッド同期経路は起動時の `resolve_projects`（窓が無いので固まらないが、到達不能ホストのぶん起動が遅れる）— 窓を先に出して接続を背景化するのが次の一歩

## 2026-09-05 — Mac 標準のテキスト編集キー一式（行頭/行末・文書頭/末の選択と削除・emacs 風）
- やったこと: ユーザー要望「行頭/行末移動はある？便利なものは全部・ACP の入力欄でも」。移動は既にあった（⌘←→ / Home End / ⌃A ⌃E / ⌘↑↓）ので、無かった側を足した。editor_core に `delete_word_forward` / `delete_to_line_start` / `delete_to_line_end`（行頭/行末で押すと改行を消して行結合＝Zed/emacs と同じ・unit test 3 本）。editor_view に `SelectToLineStart/End` `SelectToStart/End` `DeleteWordForward` `DeleteToLineStart/End`。keymap（mac）: ⇧⌘←→ ⇧Home/End ⇧⌘↑↓ ⌘⌫ ⌃K ⌥Delete + ⌃N/P/F/B/D/H（既存 Move*/Delete/Backspace に束ねただけ）。composer は同じ `Editor` コンテキストなので自動で効く。Windows は ctrl-shift-home/end・ctrl-delete に変換、⌘⌫/⌃K は VSCode に既定が無いので ctrl-shift-backspace/delete（necoder 独自）、emacs 風は ctrl-p/f/d/h と衝突するので NON_MAC_DROPPED。ショートカットシート + locales ja/en + UI-SPEC §9 のキー行を更新
- 学び/罠: keymap_core の 2 本の test（衝突・到達性）が非 mac 側の抜けを機械的に止めてくれる — 新キーは必ず「落とす / 置き換える / そのまま」のどれかを表に書く。`ctrl-k` は Windows ではチョード前置（`ctrl-k ctrl-i`）なので単発バインドにできない
- 次: 実機で composer（IBM Plex Sans JP）と markdown で一通り叩く。特に ⌃K の行結合と ⌘⌫ の IME 変換中挙動（marked_range 中は OS が先に取る想定）

## 2026-09-06 — 起動時の Remote SSH 接続をやめる（識別と接続を切り離す）
- やったこと: 前日の続き。窓が出るまでに remote project 1 件あたり ①SSH 接続（ControlMaster + server 配備 + Hello + OpenProject）②`Worktree::with_host` の canonicalize / metadata / `.gitignore` 読み ③`TaskSpace::for_worktree` の git 4 発 ④`terminal_launch` ×2（中で `ensure_master`＝接続そのもの）が**全部 UI スレッド**で走っていた。方針は「識別と接続を切り離す」— 保存状態には URI と正規化済み root が既に入っており、名乗るのに接続は要らない。**host**: `SshTransport::new`（master を起こさない）/ `SshConnector` の server 配備を初回接続まで遅延（`server_ready`）/ `ReconnectingClient.current` を `Option` にして未接続で開始できるように（初回接続と再接続を `connect_now` に一本化）/ `RemoteHost::lazy_ssh`（URI + 保存 root だけで作る。project_id=0 で始め、既存の「unknown project id → OpenProject し直す」経路に乗る）。**project**: `Worktree::trusted`（渡された root を信じ 3 往復を省く）+ `ignore` を `RefCell` 化して `set_ignore_source` で後入れ / `ProjectSource::restored` フラグ。**workspace**: 復元 slot は `TaskSpace::for_restored_worktree`（id は `stable_worktree_id_on` = host id と root のハッシュ**だけ**の純粋関数なので接続なしで正しい値が入る＝storage 鍵がずれない。git 依存項目は後入れ）/ 端末は `TerminalLaunch::default()` で置いて後で `reset_launch` / `hydrate_restored_projects` が窓表示後に「疎通確認 → `.gitignore` → task_space → 端末 → ツリー」の順で埋める。**necoder**: 復元だけ `restore_ssh_project`（local と、path 空の URI は従来経路）。検証: 新テスト `restored_projects_touch_no_host_before_the_window_is_up`（**構築が返った直後**の呼び出し回数を構築クロージャ内で控える）が 0 を固定。退行を入れると 3 で落ちることを確認。全 test 3 連緑 + fmt 緑
- 学び/罠: **`cargo test` の「parked 前＝同期」は成り立たない**。`add_window_view` を抜けた時点で背景 spawn の一部が既に走っており、前日の測り方（parked 前の回数を見る）だと背景ぶんが混ざる。構築クロージャの**中**で控えるのが正しい測り方。もうひとつ、`RemoteHost::terminal_launch` が `ensure_master()` を呼ぶ＝**launch spec を組むだけに見えて実は接続**。session 作成で 2 回呼ばれるので、ここだけで起動が止まる。**未修正が残っている**: エクスプローラのツリー再構築（`ProjectSlot::refresh` → `ExplorerProject::refresh` → `read_dir`）は **14 箇所から UI スレッド**で呼ばれ、remote では毎回 SSH の往復になる（展開・折り畳み・リネーム・watch イベントのたび）。これがあるため `assert_off_main_thread` の既定は panic ではなく警告に落とし、`NECODER_STRICT_MAIN_THREAD_IO=1` を付けたときだけ落とすようにした。潰し終わったら既定を strict に倒すこと
- 次: ①エクスプローラ再構築の背景化（14 箇所・`Worktree` は `Rc` で background へ送れないので `host` + `root` を取る自由関数側に寄せる）②metal のある環境で実機確認 — remote を刺したまま再起動して窓が即出るか / レールの並びと色が前回どおりか / 繋がるまでの見え方（未接続の表示はまだ無い＝空プロジェクトに見える。UI-SPEC に接続中/未接続の状態を足すのが UI 側の残件）③到達不能ホストで起動して、トーストが出た上でアプリが使えるか

## 2026-09-07 — SSH 切断からの復帰: 検知を速く・巻き添えを止める・残りの UI スレッド I/O を潰す
- やったこと: ユーザー報告「workspace で作業中、しばらくすると（スリープ復帰など）SSH が切れ、再接続がうまくいかずフリーズする」。前 2 日の続き。①**host 側の再接続ロジック**: Ping の timeout を専用 `PING_TIMEOUT`=5s に（従来 30s ＝ 切断に気付くまで 30s）。`RequestTimeout` 型を導入して「相手が黙っている」と「閉じた」を区別。`RpcClient` に `CountingReader` で受信バイト数を持たせ、時間切れでも受信が進んでいれば「混んでいるだけ」として張り直さない（`heartbeat_once` / `request_inner` の両方）。`connect_now` は見限る接続に `abort`（pending 全部を即エラー + session プロセス kill）してから張り直す — 以前は古い接続に乗った request が各自の timeout（最大 30s）まで待ち、書き込みで詰まったスレッドは writer ロックを握ったまま ssh の自滅（ServerAlive 45s）を待っていた。`SshConnector::connect` の master 作り直しは **Hello が `RequestTimeout` のときだけ**（即 EOF は remote-server 側の問題で、master を殺しても直らず端末を巻き添えにするだけ）。`Connector` trait を切り、UnixStream 対の `PairConnector` で再接続を ssh 無しでテスト（3 本: 時間切れの型 / 死んだ接続の待ち人が張り直しで即・透過的に成功 / 混んでいる接続を切らない）。②**`terminal_launch` / `spawn_process` から `ensure_master` を外す**: session を `ControlMaster=auto` + master と同じ alive/timeout オプション（`MASTER_OPTIONS`）で起こす＝ master が無ければ自分が master になる。launch spec を組むだけの関数がネットワークに触らなくなった（端末を開く・LSP/ACP を起こす経路が UI スレッドでも固まらない）。`ensure_master` 自体にも `assert_off_main_thread` を掛けた。③**UI スレッドの remote I/O を潰す**: エクスプローラ再構築 14 箇所 → `Workspace::refresh_explorer_for`（local は同期のまま・remote は `directories_to_read` → 背景 `project::read_listings_on`（host + root + `ignore_snapshot`）→ `apply_listings`。手元のキャッシュで `rebuild_rows` を先に当てるので折り畳み/既読展開は即時。世代 `begin_refresh`/`is_latest_refresh` + 宛先一致で古い結果を捨てる。root が読めない＝接続断のときは手元の表示を残す）。`ExplorerProject::refresh` も同じ plan/read/apply に組み替え（`build_rows` が読み直していた二重読みも消えた）。watch イベントの開きタブ照合 `EditorView::handle_external_change` → remote は `handle_external_change_remote`（背景で metadata → 必要なら read_file → 前景で revision/dirty を再確認して `apply_reloaded`）。`transition_task_space` の `git rev-parse HEAD` → remote は背景で取ってから台帳へ（遷移・ニュース・総括は待たせない。記録の phase は遷移時点のまま）。`AgentPanel::start_session` の `resolve_command_on`（remote は `command -v` ×2 の往復）→ 背景タスク内へ。新テスト `expanding_a_remote_directory_never_blocks_the_ui_thread`（退行を入れて落ちることを確認済み）。検証: host 16 test 緑 / workspace 39 緑 / explorer・project・editor_core・editor_view・agent_panel 緑 / fmt 緑
- 学び/罠: **切断検知は「時間切れ」だけでは判定できない** — 低速回線で大きな応答が流れている最中は Pong がその後ろに並ぶだけ。受信バイトの進みを見る。**張り直したら古い接続は殺す** — 殺さないと、そこに乗っていたスレッドはタイムアウトまで帰らず、writer ロック越しに heartbeat まで巻き込む。**`ControlMaster=auto` の session は `ensure_master` を要らなくする** — 端末は対話的なので、master 不在時に自分で繋ぐ方が認証プロンプトも出せる。**まだ UI スレッドに残る remote I/O**: `open_folder_in_rail`（`Worktree::with_host` 3 往復 + `TaskSpace::for_worktree` の git 3〜4 発 — 接続直後なので固まりはしないが RTT×7 の間は止まる）、`toggle_split` の `Buffer::from_host`、`revert_hunk` の `head_text_on`、`hot_exit` 復元の `open_file_sync`、`open_folder_as_window`/`open_dir_in_rail` の `host_for_project`、CLI の `ssh://` 引数での `new_sources_with_active`。`assert_off_main_thread` の既定は引き続き警告（strict に倒すのはこれらを潰してから）。この環境は `xcrun metal` 不在で necoder bin はビルド不可・Docker も停止中で `remote_ssh_live` は未実行
- 次: metal のある環境で実機確認 — remote を開いたまま Mac をスリープ→復帰 → ①5〜10 秒以内に heartbeat が張り直す（stderr に UI スレッド警告が出ない）②復帰直後にフォルダ展開・ファイル保存・エージェント送信をしても UI が止まらない ③端末タブを新規に開くと master 不在でも繋がる（`ControlMaster=auto`）。**Docker の実 SSH 検証は同日の次エントリで実施済み**。残りの UI スレッド I/O は上記「学び」の一覧

## 2026-09-07 — ヒットテストを描画と同じ run 構成にする（9/5 の「ずれ」の残りを潰す）
- やったこと: 9/5 はフォント不一致（イベント中の `window.text_style()` = OS 既定フォント）を `EditorView::text_font` で直したが、ヒットテストは**単一 run** で shape していて「Guguru Sans Code は Bold も Regular と advance が同一だから合う」という前提に寄りかかっていた（Italic の run・フォント差し替え・Bold 幅が違う family では再びずれる構造）。本文 1 行の shape を `shape_body_line`（`build_line_runs` + `shape_line`）に一本化し、prepaint と `offset_for_position`（クリック・ドラッグ・hover・IME の `character_index_for_point`）の**両方がここを通る**ようにした。ヒットテスト側のハイライト span は、prepaint が可視範囲ぶん問い合わせたものを `VisibleHighlights`（版 + byte 範囲 + span 列）として view に控え、「同じ版で・行を丸ごと覆っている」ときだけ再利用、それ以外（viewport 外へのドラッグ・編集直後の未描画）はハイライタに問い直す＝ドラッグのたびに全文を `String` 化しない。テスト 2 本（`build_line_runs` が見出し=Bold/強調=Italic の run を span どおりに立てる・`VisibleHighlights` の再利用条件）。editor_view 7 test 緑・workspace/agent_panel check 緑・fmt 緑
- 学び/罠: ヒットテストの正しさは「描画と同じ関数を通す」ことで担保するのが唯一確実で、フォントの性質（等幅・Bold 同幅）に依存する前提はいずれ破られる。`Entity::read` の借用（`wrap_map`）はループ後に切れるので、frame 末尾の `self.editor.update` で控えられる。この環境は `xcrun metal` 不在で necoder bin はビルド不可（既知）
- 次: metal のある環境で実機確認 — markdown の見出し行・`*強調*` 混じり行・日本語 + URL 混在行の途中クリックでキャレットがその文字に乗る / ドラッグ選択の終端が指の位置と一致する / composer（IBM Plex Sans JP）で同じ。エクスプローラ再構築の背景化（9/6 の残件）は別件として未着手

## 2026-09-07（2）— 実 SSH で無言断からの復帰を確かめる / 更新確認を定期化
- やったこと: ユーザーから「Docker 起動できる環境です」「0.1.11 に更新と出ているが 0.1.12 になったら追従するか。常に最新にしてほしい」。①**実 SSH 検証を実際に回した**（前エントリの積み残し）。この機械には zig/cross が無く、本体 workspace は gpui（zed の git 依存＝巨大 clone）を引くので、**crates/host + crates/paths だけの最小 workspace を作って Docker 内でビルド**する形にした（`rust:1.95.0-alpine`＝ arm64 なら既定 target が musl・2.5MB の静的バイナリ）。この手順を `scripts/test-remote-ssh-docker.sh` に取り込み、artifact が無ければ自分で建てるようにした（version は workspace の Cargo.toml から拾う — client が `<version> protocol=<n>` の完全一致を検査するため。dep のバージョン指定はわざと緩くして drift を作らない）。結果 **9/9 緑**（往復 read_file avg 0.180ms / remote-server RSS 2.2MB / idle CPU 0.00%）。②**無言断の実 SSH テストを新設**（`remote_ssh_recovers_from_a_silently_dead_link`）。`docker compose pause` で **TCP を張ったまま相手を黙らせる** — ControlMaster kill（EOF が届く）では再現できない、sleep 復帰そのものの形。12 秒凍結 → 解凍 → **ユーザーが何もしなくても 68ms で読める**ところまで自動復帰。`RemoteHost::debug_reconnect_generation` を足して「たまたま元の接続が生き返った」ではなく**張り直した（世代 1）**ことを固定した。凍結解除は Drop 番人で必ず行う（panic しても後続 test を巻き込まない）。③**切断中の積み上がり抑制**: 到達不能なホストへの再接続は 1 回十数秒（Hello 10s → master 作り直し 10s）。待たされていた request がそれぞれ最初からやり直すと「切れている間は何をしても終わらない」になる。`connect_now` の入口で時刻を控え、**自分が待っている間に起きた失敗**なら再試行せずそれを返す（到着より前の失敗＝ユーザーの新しい操作なら繋ぎに行く＝復旧を止めない）。unit test で「4 人待って試行 1 回」「その後の操作は試行 2 回目」を固定。④**更新確認を定期化**: 従来は起動 10 秒後の 1 回きり＝開きっぱなしだと開き直すまで新版が見えない。6 時間ごとに見直す。**経過は壁時計（`SystemTime`）で測る** — macOS の sleep 中は単調時計が止まり、単一タイマーだと「8 時間寝ていた」が経過に入らない。判定は純関数 `update_check_due` に切り出して test（未確認 / 間隔未満 / ちょうど / sleep 明け / 時刻逆行）。適用中・再起動待ちの状態は新しい告知で潰さない。検証: host 17 + 実 SSH 9 緑 / workspace 40 緑 / fmt 緑
- 学び/罠: **`docker pause` は「無言の死」の良い再現**だが、解凍すると凍っていた TCP がそのまま生き返るので、**復帰時間だけを見ると再接続を通らなくても緑になる**（実際 1 回目は 83ms で通り、何を測ったのか分からなかった）。世代を露出して「張り直したこと」を assert して初めてテストになる。**GitHub の `/releases/latest` は created_at 順**（semver 順ではない）。版比較は既に数値ごとなので 0.1.9 → 0.1.10 → 0.1.11 → 0.1.12 は正しく進む。**この作業ツリーは 0.1.9 だが公開済みの最新は v0.1.11**（2026-09-04 公開・別マシンからのリリース）で、`git ls-remote origin main` は `9ba6e9c`、手元の origin/main は `d7dd9e9` ＝ **手元が 2 リリースぶん後ろ**。今回の SSH 修正は 0.1.9 の上に載っている（合流はユーザー判断）
- 次: ①metal のある環境で実機確認（前エントリの ①②③）②`assert_off_main_thread` を strict 既定へ倒す前に、残る UI スレッド I/O（`open_folder_in_rail` の `Worktree::with_host` + `TaskSpace::for_worktree`、`toggle_split`、`revert_hunk`、hot-exit 復元、`host_for_project`）を潰す

## 2026-09-07（3）— origin/main（v0.1.10 / v0.1.11）と合流
- やったこと: ユーザーが `git pull` して「local changes would be overwritten」で中断（作業は無傷・git が守った形）。手元の未コミット作業を `fix/remote-ssh-reconnect` に確定させてから `origin/main`（8 commit・38 files・+2226）を merge。**衝突 6 件の判断**: ①`build_step_entry`（agent_panel）= 向こうの heredoc 圧縮表示（`cat > file <<EOF` → `書き込み file`）と手元の出力打ち切り（`cap_output` が行数も返す・`result_lines`）は**別機能なので両立**させた ②`necoder/main.rs` の復元 = 向こうの**複数窓復元**（`claim_window_sessions` → `restore_window_plan`）を採り、その中の SSH だけ手元の**遅延接続**（`restore_ssh_project` = 繋がずに開く）へ差し替え。向こうの `connect_ssh_project` のままだと到達不能ホストで起動が遅れる ③`project_session.rs` = `save_state(cx)`（向こうの新シグネチャ）と `hydrate_restored_projects`（手元）を両方残す ④CHANGELOG = 手元の Unreleased を 0.1.11 / 0.1.10 の上に積む ⑤UI-SPEC のステップ行 = 両方の記述を 1 行に統合 ⑥JOURNAL = 日付順（向こう 09-02〜09-04 → 手元 09-05〜09-07）。**自動 merge の意味的な穴を 2 つ発見**: `UpdateState::Installing` が向こうで進捗を持つ tuple variant になっており、手元の定期確認のパターンマッチが型エラー（コンパイラが捕捉）。向こうが新設したエクスプローラ手動更新ボタン（↻）が `slot.refresh()` を同期で呼んでおり、**remote で押すと固まる**経路が復活していた（`refresh_explorer_for` へ寄せた・コンパイラは捕まえない）。version は 0.1.11 になり、更新チップの比較対象も自動で追従する
- 学び/罠: **merge 後は「型が通ったか」だけでなく「同じ規律を破る経路が入っていないか」を grep する**。今回は `slot.refresh()` / `.explorer.refresh(` で全 call site を数え直して 1 件見つけた。並行セッションが同じ領域に機能を足すと、こちらが潰した経路が別の入口から戻ってくる。`assert_off_main_thread` の既定を strict に倒しておけば実行時に捕まえられたはずで、倒す価値がまた上がった
- 次: ①`main` を branch へ早送り（`git checkout main && git merge fix/remote-ssh-reconnect`）— ユーザー判断 ②metal のある環境で実機確認 ③残る UI スレッド I/O を潰して `assert_off_main_thread` を strict 既定へ

## 2026-09-07 — v0.1.12 リリース / Windows CI: notify の監視スレッドが gpui テストを落とす
- やったこと: CHANGELOG の Unreleased を 0.1.12 へ繰り上げ・`workspace.package.version` を上げて `v0.1.12` を push（Release workflow が dmg/zip を添付）。同時に走った main の CI で **check-windows の Test だけ**が落ちた: `expanding_a_remote_directory_never_blocks_the_ui_thread` が「Detected activity on thread `notify-rs windows loop` … Your test is not deterministic」。gpui のテストスケジューラは別 OS スレッドからのタスク wake を非決定的と判定して `end_test` で panic する。各テストは teardown で `_watch = None` にしてから root を消していたが、watcher の drop と監視スレッドは非同期で競合が残る（mac は FSEvents の遅延で偶然通っていた）。直しは `project_watcher::file_watching_enabled()`（`!cfg!(test)`）で **テストビルドでは監視自体を張らない**。workspace 44 test 緑（mac）
- 学び/罠: gpui の `#[gpui::test]` の中で **OS の監視スレッド（notify）を生かしたまま fs を触ってはいけない**。teardown で落とす回避は「落とす順」では解決しない。Zed が FakeFs を使うのと同じ理由。テストは watch イベントに依存していない（`handle_watch_events` を直接呼べる）
- 次: Windows CI が緑になるのを確認。v0.1.12 は失敗前のコミットに付いており、失敗はテストのみ（成果物には影響しない）

## 2026-09-07 — プロジェクト色を永続化する（並び順で色が変わるのをやめる）
- やったこと: ユーザー報告「再起動したら色が変わる。プロジェクトの色は設定として持つべき」。調べると手動で選んだ色だけ `.necoder/settings.json` に残り、自動割当はレールの添字で `project_color(index)` を巡回しているだけだった（`project_session.rs` の slot 構築）。外す・別窓に単独で開く・窓セッションが飛ぶ、のどれでも全部ずれる。リモートはホスト単位 `host_colors` で焼いていたので Macstudio の 4 プロジェクトが全部同じマゼンタ。①`storage.project_colors`（scope=local/ホスト鍵 + root パス → 0xRRGGBB・`load_project_colors` 一括読み + `project_color`/`set_project_color` upsert）を新設し `host_colors` の API と CREATE を撤去（残った表は無害）。②`workspace/project_colors.rs`: `resolve_project_colors` を `new_sources_with_active` の **session 生成前**に呼ぶ（storage の open を前へ移動。TodoPanel 等の accent が slot.color から作られるため後から直すと初期値がずれる）。優先順 `.necoder`（`ProjectSlot.identity_color` に保持）> DB > 未使用パレット色（pure fn `assign_free_colors`）。書くのは値が変わる行だけ。③レールへ足す `add_project_slot` は 親の色 > `.necoder` > DB > `next_free_color` の順にして `persist_project_color`。④手動選択 `apply_project_color` はローカルなら `.necoder` + DB、リモートは DB。⑤`theme_core::hex_from_color`（`color_from_hex` の逆・往復 test）。⑥「最近」のホスト鍵計算を `host_scope_key` に一本化。⑦リモートの `.necoder` は読まない（同名ローカルパスの誤読み防止・doc の意図どおり）。docs: UI-SPEC §1.2 優先順・ARCHITECTURE・CHANGELOG Unreleased・ROADMAP M10。
- 学び/罠: **決まった色は同窓で衝突しても変えない**（「いつも同じ色」が識別の前提。衝突は手動で直せる）。自動割当だけが避ける。gpui テストで `.necoder/settings.json` を `r#"…"#` で書くと `"#d946ef"` の `"#` で raw string が閉じる（`r##"…"##` にする）。macOS の temp_dir は Worktree が `/private/var` に canonicalize するので DB の鍵と比べるならテスト側も canonicalize。このマシンは Metal が無く `cargo check -p necoder` は gpui_apple の build script で止まる＝crate 単位（workspace/storage/theme_core）で check/test する。
- 次: metal のある環境で実機確認 — 17 プロジェクトを開き直して色が前回どおりか / レールで色を選び直して再起動後も残るか / Macstudio の cosma・oshinoma・hf-copilot が別色になるか。state.json からの旧セッション移行は入れない（ユーザー判断: 常に最新で使う前提・古い版の救済はしない）。

## 2026-09-07 — Remote SSH ターミナルの `ne .` を接続元へ戻す
- やったこと: ユーザー実機で、necoder から Macstudio へ Remote SSH 接続した統合ターミナル内の `ne .` が「necoder で開きました」と成功表示するのに接続元では開かなかった。8/30 の修正は同一マシンの `gui.sock` と LaunchServices だけで、SSH 境界を越える戻り道が無かったため、接続先 Mac の `/Applications/necoder.app` を起動していた。①各 `SshTransport` に接続元の open 専用 Unix socket gateway を用意し、ControlMaster の `-R remote.sock:gateway.sock` で reverse forward（owner-only socket・stale unlink）。②remote terminal に `NECODER_GUI_SOCK=remote.sock` を注入し、既設の `ne` も接続元へ向くようにした。GUI 版が無い Linux でも使えるよう、配備済み `necoder-remote-server cli` を呼ぶ軽量 `ne` shim を一時 PATH に置く。③gateway は remote の要求を `open` に限定し、接続先の SSH authority を**ローカル側で**刻んで GUI の `open_remote` へ変換。`gui.sock` を直接 reverse forward して fleet/send 等の全権限を remote へ漏らす設計は採らない。④GUI は remote path を `ssh://authority/path` にして既存 `connect_ssh_and_open` へ合流。remote-server の機能追加を古い配備物と混ぜないため protocol 2→3。⑤unit 2 本（open の変換/応答中継・他 method の拒否）+ 実 SSH test `remote_terminal_ne_opens_on_the_client` を追加。
- 検証: `cargo test -p host`（19 unit + daemon integration 緑）、`cargo test -p necoder --features runtime-shaders`（5 unit + manifest path 3 緑）、同 feature の check 緑。さらに実 `Macstudio` / `/Users/daichi/Work/experience/new-vc` へ p3 server を自動配備し、remote login shell へ `ne .` を入力 → reverse socket → 接続元の隔離した偽 GUI で `method=open_remote`・`authority=ssh://Macstudio`・remote 絶対 path の一致 → 応答が remote shell まで戻る一往復を実証。本物の GUI/DB は触っていない。同じ test を使い捨て Ubuntu/OpenSSH Docker でも実行し、**デスクトップ版も既設 `ne` も無い Linux**で配備サーバの軽量 CLI が同じ一往復を通すことも確認。protocol 更新後の古い Linux artifact を拾わないよう Docker harness は host source より古い cache を再ビルドする。
- 学び/罠: 「既存ウィンドウへ開く」と「SSH 接続元へ戻す」は別問題。`SSH_CONNECTION` だけではどの alias/user/port で開いた workspace かも、接続元 GUI の IPC 口も分からない。Remote SSH を作ったクライアント自身が reverse channel と authority を渡す必要がある。また、汎用の制御 socket を reverse forward すると open の便利さと引き換えに remote process へ管制権限を渡すので、能力を open だけに絞る gateway が境界になる。
- 次: 新ビルドを `/Applications` へ入れた後、実 GUI の Remote SSH ターミナルで `cd new-vc && ne .` を押し、接続元ウィンドウのレールに new-vc が追加されることを本人が最終目視。既に開いている terminal は古い SSH command のままなので、新ビルド後に terminal tab を作り直す必要がある。

## 2026-09-07 — UI スレッドの remote I/O を潰し切る（reconnect の最後の層）
- やったこと: ユーザー「まだ未実装？実装しきって」。9/7 のエントリで「まだ UI スレッドに残る」と書いた 6 経路を全部背景へ。①`open_folder_in_rail`: remote は `Worktree::with_host`（canonicalize / metadata / .gitignore）と `TaskSpace::for_worktree`（git 3〜4 発）を 1 本の背景タスクで済ませ、`Worktree` ごと返して前景で slot にする（`Worktree` は `Rc` を持たないので background から返せる。`Rc` なのは `ProjectSlot` 側）。後半を `add_worktree_to_rail` に切り出し、local は従来どおり同期。読んでいる間に同じ root が載っていたら切替だけ（`switch_to_open_root`）。②`open_folder_as_window` / `open_dir_in_rail`: remote の `host_for_project`（OpenProject の往復）を背景へ。新窓は `ProjectSource::restored`（root は背景で canonicalize）で組み、`hydrate_restored_projects` が接続と中身を後から埋める＝窓を出すのに接続を待たない。レールの tear-off も remote なら同じ信頼経路。③`toggle_split`: remote は `read_file` を背景で読んでから `Buffer::from_content`。読んでいる間に分割/タブが変わっていたら捨てる。④`revert_hunk`: `head_text_on`（git の呼び出し・remote なら往復）を `stage_hunk` と同じく背景へ、置換だけ前景。⑤hot exit 復元: remote は `open_file_then`（開き終わってから流し込む合流点）へ。local は同期のまま（タブ順を保つ）。⑥`necoder ssh://host/path` 起動: `restore_ssh_project` の遅延接続に乗せて窓を先に出す（home＝path 無しだけは root を知るために同期で繋ぐ）。⑦`assert_off_main_thread` の既定を **debug は panic・release は警告** に（main.rs のコメントは元からそう書いてあり、実装が追いついていなかった）。回帰テスト 2 本（`adding_a_remote_project_to_the_rail_never_blocks_the_ui_thread` / `splitting_a_remote_file_never_blocks_the_ui_thread`・`BlockingIoAuditHost` で「呼び出し直後の I/O 回数が増えない + parked 後に結果が出る」）。workspace 51 / host 19 / project 25 緑・fmt 緑
- 学び/罠: **「残っている」と書いた一覧は、そのまま次の仕事のチェックリストになる**（9/7 の「学び」の一覧を上から消化しただけ）。`Worktree` を background から返せることに気付くと、`with_host` を分解せずに済む（9/6 は「Rc で送れない」と書いたが、それは `ProjectSlot` の話だった）。remote の新窓は「復元」と同じ遅延経路に寄せるのが一番安い＝接続を待たずに窓を出す仕組みは 1 本でよい。この環境は Metal 無しで necoder bin は check 不可（main.rs の変更は目視・`restore_ssh_project` は既存関数）。
- 次: metal のある環境で実機確認 — ①remote を開いたまま Mac をスリープ→復帰→レールに別の remote フォルダを足す / 分割 / hunk 巻き戻しで UI が止まらない（debug ビルドなら違反は panic で即分かる）②`necoder ssh://Macstudio/path` で窓が即出て中身が後から埋まる

## 2026-09-07 — /compact ボタンを縮小アイコンから文字チップへ
- やったこと: ユーザー「Compact が縮小アイコンで分かりづらい、やめろと何回言えば分かる」。agent panel の meta 行（トークンメーター横）の文脈圧縮ボタンが bb91901 の追加時から `minimize.svg`（⤡）のままで、何が起きるか読めなかった。送信待ちの「中断して今すぐ」と同じ小チップ様式（border・10.5px・rounded 5）で、送るコマンド名そのもの `/compact` を monospace で出す形に変更。`agent.compact` を `locales/{ja,en}.yml` に追加（両言語とも `/compact`）。右クラスタの固定幅 174→214px、非表示時の占有スロットも 18→64px（猫とメーターの位置不変の規律はそのまま）。`cargo check -p agent_panel` / `cargo test -p agent_panel`（30 緑）/ `cargo test -p i18n` 緑。`minimize.svg` 自体は fleet のセル縮小で使うので残す
- 学び/罠: **操作ボタンの意味がアイコンから読めないなら文字で出す**。特にコマンドを送るだけのボタンは、コマンド名を出すのが一番誤解がない。同じ指摘を複数回受けていたのに記録が無く放置されていた → memory（`compact-button-no-shrink-icon`）に禁止事項として残した。この環境は Metal 無しで screenshot 不可
- 次: metal のある環境で `./scripts/screenshot-app.sh` → meta 行のチップの幅（64px に `/compact` 8 文字が収まるか）と、ja/en で猫が横に跳ねないことを目視

## 2026-09-07 — 初回ターンだけ Fable が答える（モデル/思考量の起動時レース）
- やったこと: ユーザー報告「既定は Opus 5 のはずなのに勝手に Fable が使われる。表示と違う。ACP を立ち上げた初回になりやすい」。8/28 に権限モードで潰した「遅延起動では UI の後追いコマンドが Prompt の後ろに並ぶ」レースが、Model / Effort には残っていた。`send_prompt_text` はセッション起動直後に `Prompt` をキューへ積む → `run_session_on` は `session/new` の `config_options` を UI へ流してすぐ Prompt を読む → UI が `Configs` を受けて送る `SetConfig` はもうターン中で `deferred` 行き → **初回ターンは必ずエージェント既定モデル**で走り、2 ターン目から Opus。ピルは sticky を楽観表示するので「表示は Opus・応答は Fable」。直し: ①`acp_client::SessionPreferences { mode, model, effort }` を新設し `run_session` / `run_session_on` の `desired_mode: Option<String>` を置き換え ②`session/new` 直後・最初の prompt を読む前に、広告 `config_options` から希望（表示名 or value_id・大文字小文字無視）を `find_config_choice` で引き、current と違えば `session/set_config_option` を送る。応答の一覧を UI へ `Configs` として流す＝UI 側の照合が一致して二重送信しない。広告に無い希望は送らず、従来どおり UI が広告 current を採用 ③`start_session` がスレッドの `permission_mode` / `model` / `effort` を渡す。テスト: `config_choice_matches_name_or_id_case_insensitively`（純関数）+ `preferences_apply_before_first_prompt`（python の偽 ACP エージェントが受けた method を順に記録し、prompt 応答で「その時点の current」を返す → `set_config_option` ×2 が `session/prompt` より前・初回ターンが opus/max）。acp_client 19 / agent_panel 30 緑・fmt 緑
- 学び/罠: **「起動時に合わせる」対象は SessionMode だけではない**。ACP で「UI が広告を受けてから送る」型の設定は全部この順序レースを踏む（Mode / Model / ThoughtLevel）。偽エージェントのテストで `command_tx` を **ターン中に drop するとターンループが畳まれて応答を待たない**（`TurnEvent::Command(None) => break`）。`prompt_error_fails_turn_but_keeps_session` の注記どおり TurnEnded 後に drop する。`live_stream`（ignored）は起動前に drop しており同じ穴を踏む可能性がある
- 次: metal のある環境で実機確認 — 新規タブで Opus を表示したまま送信 → 初回ターンの応答モデルが Opus（/model や応答の自己申告で確認）/ effort ピルの値も初回から効く / Codex タブで GPT 系の sticky が初回から効く

## 2026-09-08 — SSH 切断後の ACP セッション再開 + statusbar に接続状態（「再接続ボタンは要るか」への答え）
...
## 2026-09-08 — 「再開」が効かず端末の開き直しで直る（SSH master が `-R` の bind 失敗で永久に落ちる）
- やったこと: ユーザー報告「`claude-agent-acp が見つかりません` が出てセッションが切れ、再開しても再開しない。なぜかターミナルを再起動すると戻る」。実機ログ（`~/Library/Application Support/necoder/logs/`）に `remote port forwarding failed for listen path /tmp/necoder-cli-….sock` → `OpenSSH ControlMaster の接続に失敗: exit status: 255` が 100 回以上並び、Macstudio 側にはその socket が起動時刻（15:11）のまま 2 本残っていた。**`start_master` の `-R`（`ne` gateway）は bind を remote の sshd が行い、古い socket を消すかは sshd_config の `StreamLocalBindUnlink`（既定 no）が決める。client 側の同名オプションは `-L` にしか効かない**。前回の master が異常終了して socket が残ると以降の張り直しは全て bind 失敗 → `ExitOnForwardFailure=yes` で master ごと exit 255 → 再接続が永久に失敗。端末・ACP の standalone session（`ControlMaster=auto`）は `-R` を付けないので、端末を開き直すとそれが master になって RPC も回復する＝「ターミナル再起動で直る」の正体。直し: **host** — master は `-R` 無しで張り、`install_cli_forward` が master 経由で古い socket を `rm -f` してから `ssh -S … -O forward -R …` で転送を後付け（失敗は警告のみ・接続は続く）。`Host::run_command_retry_safe`（既定は `run_command`）を足し、RemoteHost は `scoped_request(.., true)` で再送可に。**acp_client** — `resolve_command_on` / `command_on` を `Result<Option<AgentCommand>>` にし、「探せたが無い」(`Ok(None)`) と「探せなかった」(`Err`) を分ける。`command -v` は `run_command_retry_safe` で流す。**agent_panel** — background の解決失敗は `agent.err_agent_lookup`（原因文つき）で出し、`err_no_acp` は本当に無いときだけ。locales ja/en に追加。テスト: `resolve_on_host_tests`（偽 remote host で Err / None / Some + 再送可フラグ）
- 学び/罠: **`StreamLocalBindUnlink` は方向で効く側が違う**（`-L` は client、`-R` は sshd）。**転送を master 起動の必須条件にしない** — `ExitOnForwardFailure` は転送が接続の目的のときの設定で、付随機能（`ne`）に使うと接続本体を人質にする。「見つからない」と「探せなかった」を 1 つの `None` に畳むと、通信障害がインストール案内に化けて原因が読めなくなる
- 次: metal のある環境で実機確認 — ①remote を開いたまま Mac をスリープ→復帰→バナーの「再開」だけで「前回の会話を引き継いで再開しました」が出る ②Macstudio の `/tmp/necoder-cli-*.sock` が張り直しのたびに作り直される（残骸で落ちない）③remote 端末で `ne .` が接続元へ開く（転送の後付けが効いている）
...
## 2026-09-09 — 長いユーザー入力を畳む / エントリ操作ボタンを 28px へ
- やったこと: ユーザー報告「ACP への input が長い（行数）とタブが埋まる。コピーの横に折りたたみボタンが要るのでは。コピーボタンも相変わらず小さい」。`Entry::User` は長さに関わらず全文を描いていたので、貼り付けたログや仕様を送るとその 1 エントリで transcript が埋まり、続くエージェントの応答が画面外へ出ていた。直し: **agent_panel** — `user_entry_foldable`（>12 行 or >1000B）/ `user_entry_preview`（先頭 8 行・改行が無ければ先頭 400 文字 + `…`）と `expanded_inputs`（`(thread.id, entry index)` 鍵・`expanded_steps` と同じ流儀）を足し、長い User エントリは既定で折り畳み。展開は**本文下の文字チップ `▸ 全 N 行を表示`** と**エントリ右上の操作ボタン**の両方から。hover で出る右上のボタンは列（`▸`/`▾` → `⧉`・gap 4px）にし、寸法を **22px 角 → 28px 角**（コピーのグリフ 12.5px → 15px・hover で bg3）へ。locales ja/en に `agent.input_expand` / `input_expand_tip` / `input_collapse_tip`。UI-SPEC §transcript の msg-user 行とコピーボタン行を更新。テスト: 閾値・プレビュー（行/文字の両方で切る）5 本
- 学び/罠: **畳む判定は行数だけでは足りない** — 改行の無い長文を貼られると 1 行のまま折り返しで画面を埋めるので bytes も見る。プレビューも行と文字数の両方で切る。折り畳みの操作は**アイコン単独にしない**（畳んでいる間は「全 N 行を表示」の文字チップを本文下に出す＝押せると分かる・/compact ボタンと同じ判断）。このマシンは Metal 無しで `-p necoder` がビルドできず**スクショ未検証**（`cargo test -p agent_panel` 36 本と i18n parity は緑）
- 次: metal のある環境で目視 — ①長い入力が先頭 8 行 + チップで畳まれる ②右上の操作列（折り畳み + コピー）が本文に重なっても押せる大きさか ③展開してもスクロール位置が飛ばないか

## 2026-09-10 — Remote PWA / Cloudflare リレー / Mac・Windows ホスト
- やったこと: `relay/` に E2E ペアリング、DO WebSocket 中継、PWA、Node ホスト、通知、失効、期限と request receipt による再実行防止を実装。GUI の `remote_*` API は既存 ACP スレッドを利用し、project / thread / turn / permission の相関キーを UI 更新内で再検証。`ne remote` と Mac / Windows のホスト同梱、Windows 名前付きパイプ / ACL / ローカル管理認証を追加。`control.necoder.com` へ配置済み。詳細は `docs/REMOTE-CONTROL.md` と `relay/README.md`。
- 学び/罠: `NECODER_HOME` は GUI socket パスを変えないため、隔離テストは `NECODER_GUI_SOCK` も明示する。Wrangler の custom domain 付き dev は Origin の書換えが絡むのでブラウザテストは routes のない専用設定を使う。PWA の復帰時は新しい暗号セッションで snapshot を再取得し、未確認の命令を勝手に再送しない。QR 鍵を保存後 confirm 前に切断したケースも再接続時 confirm で回復する。
...
## 2026-09-10 — MCP サーバをスレッドへ渡す（ACP の `session/new` に載せる）
- やったこと: 発端は「Codex CLI に登録した Higgsfield が necoder のスレッドから `not installed or connected` になる」。原因は Higgsfield 側ではなく、**ACP では MCP サーバを渡すのがクライアントの仕事**なのに necoder が `session/new` に 1 つも載せていなかったこと。`acp_client::mcp` を新設し、`settings.json` の `mcp_servers`（stdio / http / sse・ヘッダと env の `${VAR}` 展開）と、他ツールの設定からの発見（`~/.codex/config.toml` / `~/.claude.json` / `~/.cursor/mcp.json`）を 1 本の一覧へ解決 → `session/new` と `session/load` の `mcpServers` へ受け渡し。渡せなかったものは `AgentEvent::Notice` で transcript に 1 行。設定画面に「MCP サーバ」セクション（出所ラベル + 伝送方式 + トグル）。`toml` を workspace 依存に追加（既に lock 済みの 0.8 系・`parse` のみ）。
- 学び/罠: ①**このマシンでも GUI はビルドできる** — `--features necoder/runtime-shaders`（`scripts/screenshot-app.sh` が metal 不在時に自動で付けるのと同じ逃げ道）。「Metal が無いから GUI は触れない」は誤りだった。②`pref_row` に `min_w_0` が無く、**長い副題がトグルを幅 0 に潰していた**（MCP の stdio コマンドは絶対パスで 100 文字超）。crate 冒頭に書いてある `flex_1` + `min_w_0` はセット、の罠そのもの。offscreen 目視で見つかった（テストでは出ない）。③この offscreen 経路は**文字がラスタライズされない**ので、確認できるのは枠・色・配置まで。文言は実アプリの目視が要る。④Codex の `[mcp_servers.x]` には `cwd` があるが ACP の `McpServerStdio` に対応する場所が無いので読まない（黙って別の意味にしない）。
- 検証: `cargo test --workspace --features necoder/runtime-shaders` が 56 バイナリ全 green（新規: acp_client 12 + settings_core 2 + 偽エージェントへの wire テスト 1）。**実 `claude-agent-acp` での live 実証** — 自作の stdio MCP を ACP 経由で渡すと `mcp__necoderprobe__necoder_probe_ping` として呼ばれ magic 文字列が返った。`claude-agent-acp` の `mcpCapabilities` は http・sse とも true。設定画面は新設の `settings_preview` example（offscreen）で目視し、上記②を発見・修正して再撮影で確認。
- 次: ユーザー環境での実操作（設定画面で higgsfield を on → 新しいスレッドで Higgsfield ツールが見えるか）。Higgsfield は OAuth 必須のリモートサーバなので、**エージェント側が OAuth を通せるか / bearer トークンをヘッダで渡す必要があるか**はそこで判明する。necoder 側は両方に対応済み（`headers` の `${VAR}` 展開）。

## 2026-09-10 — 起動直後のモデルピルが「—」になる（0.1.14 の退行）を直す
- やったこと: ユーザー報告「1.14 で初回起動時に ACP の下のモデル名が出ない。適当に送ると出る」。原因は 89f7610 で復元時のモデルを **sticky（`agent_config_defaults`）だけ**にしたこと。sticky はピルで**明示的に選んだ時しか**書かれず、ACP セッションは初回送信まで立たない＝`AgentEvent::Configs` の広告も来ないので、モデルを一度も自分で選んでいないユーザーはピルが送信までずっと `SELECTOR_UNSET`（—）だった。直し: `set_storage_inner` の復元で **sticky が空のときだけ** DB の `threads.model`（その行が最後に使ったモデル）へ落ちる（sticky は上書きしないので 89f7610 の「旧語彙が sticky を打ち消す」問題は再発しない。agent 列を持たない旧行は別 vendor の綴りを持ち込みうるので対象外）。`add_thread` も同 agent なら直前タブの model / effort を引き継ぐ（mode と同じ理屈）。表示は value_id のまま＝広告が来たら表示名へ引き直る（捏造しない）
- 学び/罠: **necoder はスレッド 1 本 = ACP プロセス 1 本の遅延起動**。Zed は逆で、`ConversationView::initial_state` が生成時に `AgentConnectionStore::request_connection` を呼び、agent ごとに 1 本の接続を**共有**して先に張る（`ServerState::Loading` の間はピル自体を描かない・`config_options.rs` は `selectors.is_empty()` で何も出さない）。つまり Zed は「起動直後は広告がある」前提で、necoder は「広告が来るまで何を出すか」を自分で決める必要がある側。今回はプロセスを増やさない側（記録から出す）に倒した
- 検証: `cargo test -p agent_panel` 38 本 green（回帰テスト `restored_thread_shows_last_used_model_before_advertisement` 追加: DB 行だけからピルが `opus[1m]` を出す・新規タブも引き継ぐ）。`cargo check -p necoder --features necoder/runtime-shaders` / `cargo test -p necoder` 緑。**目視は未**（offscreen は文字がラスタライズされないので、この変更＝文字だけの確認には使えない）
- 次: 実機で起動直後のピル表示を確認。まだ残る穴は ①新規インストール直後（DB も sticky も空）は最初の送信まで「—」 ②広告前はピルを開けない（押しても選べない）。埋めるなら Zed 流のセッション先張り（アクティブタブだけ prewarm）か、agent ごとに接続を共有する作りへの変更が要る＝idle メモリ予算との相談

## 2026-09-10 — 「閉じたタブが再起動で復活する」の残り 2 経路を塞ぐ
- やったこと: 09-03 の `state.json` → `window_sessions` で直したはずの再発報告。**閉じる→保存→復元の本線は正しかった**（往復テストを書いて確認）ので、残っていた 2 つの穴を潰した。①**復元タブ列の添字ずれ**: `new_sources_with_active` は開けなかった source を黙って飛ばすのに、`restore_open_file` は `sources` と同じ並びの添字でタブ列を配っていた＝消えた worktree / 消した Downloads が 1 つ混ざるだけで、それ以降の全プロジェクトが**隣のプロジェクトのタブ列**を受け取る（`active` も同様にずれる）。構築時に `projects[i]` の出所 source 添字（`restored_source_map`）を記録し、復元はその写像を通す。②**窓を閉じると直前の保存が捨てられる**: `WindowSessionWriter::close` が郵便受けを `take()` して捨てたうえ `closed` を立てていたので、「タブを閉じて即 ×」で最後のタブ操作が無かったことになる。`close(final_payload)` にして、**閉じ印の前に書き切る**（`mark_window_closed` は今のタブ列から payload を作って渡す）。②は自前 titlebar の × 経由＝ Windows / Linux のみが対象（mac の赤ボタンは OS フック側で、Workspace の書き手は生きたまま残るので郵便受けは流れ切る）
- 学び/罠: **並行する 2 本の Vec を添字で対応づけると、片方だけが要素を落とせる場所が事故になる**。ここでは「開けない project は飛ばす」という別の親切が、復元の添字を静かに壊していた。`install_window_close_hook` の中から `WindowHandle::update` で Workspace を触ることはできない（その窓は今まさに update 中で `update_window_id` の `take()` が None → Err。panic ではなく黙って失敗する）ので、mac 側は「閉じ印だけ付けて書き手は生かす」形が正
- 検証: `cargo test -p workspace` 54 green（新規 3 本: 閉じる→⌘Q→DB 往復 / 窓を閉じる直前の保存 / 消えた project を挟んだ復元のずれ。3 本とも修正前は落ちることを確認済み）。`cargo check -p necoder --features necoder/runtime-shaders` 緑
- 次: 実機で「タブを閉じる → ⌘Q → 起動」を一巡。①の再発監視には、レールから消えた project が居る状態での起動を見るのが早い

## 2026-09-10 — ACP セッションを先に張る（モデル一覧が起動直後に出て、選べる）
- やったこと: 前エントリの「記録から出す」では**選べない**（広告が無い＝メニューが空）ままだったので、Zed と同じ「先に繋いでから描く」に寄せた。①**先張り**: `AgentPanel::schedule_prewarm` / `prewarm_thread` を新設し、復元直後・宛先確定・タブ切替・新規タブ・agent 切替で **アクティブなスレッド 1 本だけ** のセッションを 500ms 後に立てる（世代番号で最後の予約だけ生存＝タブを流し見してもプロセスを増やさない）。失敗は**静かに**（未導入・起動失敗を transcript のエラーにしない）・1 スレッド 1 回だけ試す。Fleet の Task セル（`new_task`）は先張りしない。②**広告の在庫**: 広告は agent 固有でスレッド非依存なので `AgentPanel::catalog` に agent 別で控え、**セッションがまだ無いタブのピル**もそこから選択肢と表示名を出す（選んだ値は sticky → 次のセッションへ `SessionPreferences` で渡る＝送る前に選べる）。③**Agent ピルの固定条件**を「セッションが在る」から「会話が始まった（entries / 実行中）」へ。先張りで固定されると起動しただけで Agent が選べなくなるため。切替時は前の agent のセッション・広告・session id を畳んで張り直す。④設定 `agent_prewarm`（既定 on・設定画面にトグル + ja/en）。ARCHITECTURE §7.3 と UI-SPEC のピル契約を更新
- 学び/罠: **`session/new` の時点でモデルは広告される**ことを実機で確認した（新設 `cargo run -p acp_client --example probe_advertisement`。prompt を 1 通も送らずに Model 5 / ThoughtLevel 6 / Mode 5 が届く。current は `fable[1m]`）。ここが false なら先張りしても無意味だったので、設計の前提は**先に実機で確かめる**のが正しかった。もう 1 つ: 先張りは `agent_selector_locked` の判定条件を壊す（`command_tx.is_some()` を「会話が始まった」の代理にしていた）。**代理の条件は、前提が変わると静かに嘘になる**
- 検証: `cargo test --workspace --features necoder/runtime-shaders` 56 バイナリ全 green（agent_panel は 40 本 = `prewarm_targets_only_the_visible_thread` / `advertisement_stock_feeds_tabs_without_a_session` を追加）。**実アプリで end-to-end 確認**: 隔離 `NECODER_HOME` で起動 → **prompt を 1 通も送っていないのに `claude-agent-acp` が 1 本だけ**立つ（親 pid = necoder）→ 終了で残骸 0。ピル**文字**の目視は未（全画面キャプチャは他ウィンドウしか写らず、撮影は本人環境の手番）
- 次: 本人の実機で ①起動直後にモデル名が出て**押せる** ②Codex スレッドでも同じ（codex-acp がこのマシンに無く未確認） ③タブを何枚も開いた時のプロセス数とメモリ（`scripts/memory-usage.sh`）。プロセス数が気になるなら Zed 流の「agent ごとに 1 接続を共有」へ寄せる余地がある（今は thread 1 本 = プロセス 1 本）

## 2026-09-11 — 先張りの上限（見ただけのタブの分までエージェントを常駐させない）
- やったこと: 先張りの代償「見たタブの数だけプロセスが立つ」が気になる、との指摘。まず**実測**（本人の稼働中 necoder・RSS 104MB にぶら下がる 4 セッション）: 未送信のセッションは adapter + 子で **約 16MB**、1 度使うと約 46MB、会話が育つと約 244MB。増えるのは「見ただけのタブ × 16MB」だけと分かったので、そこに上限を置いた: `MAX_IDLE_PREWARMED_SESSIONS = 2`、`Thread::session_used`（prompt を 1 通でも送ったか）、`AgentPanel::prewarm_order`（先張りで立てた順の台帳）、`retire_idle_prewarms`。未使用・非実行・承認待ちでないセッションだけを古い順に畳む。2 本なのはタブを A→B→A と行き来しても立て直しが起きない最小の数だから
- 学び/罠: 畳むときは **`session_serial = 0` に戻す**のが要点。そうしないとポンプ終了の後始末（`session_ended`）が「今のセッションが落ちた」と判断して composer 直上に切断バナーを出す＝**意図して畳んだのに事故に見える**。`acp_session_id` は残すので、そのタブへ戻った時は `session/load` で続きに戻れる。畳んでもピルの選択肢は在庫（`AgentPanel::catalog`）から出続けるので、見た目は何も変わらない
- 検証: `cargo test --workspace --features necoder/runtime-shaders` 56 バイナリ green（agent_panel 41 本・`idle_prewarmed_sessions_are_capped` 追加: 使ったセッションは畳まず、未使用の最古だけ畳み、`session_serial = 0` と再先張り可能になることまで）。ARCHITECTURE §7.3 に上限と実測値を追記
- 次: 本人の実機で `scripts/memory-usage.sh` — タブを何枚か行き来した後の idle RSS が 0.1.14 比でどれだけ増えるか（想定 +16〜32MB）。それでも重いなら Zed 流「agent ごとに 1 接続を共有」（`acp_client` を接続アクター化し 1 プロセスで複数 session を多重化）が次の段

## 2026-09-11 — HTML プレビュー中に ACP で日本語が打てない（first responder の取り返し）
- やったこと: 報告「HTML プレビューを使うと ACP で日本語入力ができず、ACP 外で何か動く」。原因は**ネイティブ WebView が macOS の first responder を握りっぱなし**になること。①necoder は HTML タブを選ぶと `webview.focus()`（wry 実体 = `makeFirstResponder(webview)`）②GPUI 側にこれを取り返す仕組みは無い（`gpui_macos` が `makeFirstResponder` を呼ぶのは窓生成時の 1 回だけ・`window.focus()` はアプリ内部のフォーカスを動かすだけで OS には何も言わない）③necoder が返していたのは「隠す / 破棄する」時だけ。**症状が日本語に偏る理由**: gpui の文字入力は `[self inputContext] handleEvent:` を通り、`inputContext` は first responder でないと生きない＝ IME の変換セッションは WebView 側に付く。一方 IME が食わない生のキーは responder chain を上って GPUI に届くのでキーマップだけ発火する。直し: `WebViewView::set_key_focus(owns)` を新設（表示状態は変えずフォーカスだけ渡す / 返す）、`Workspace::release_native_key_focus` を **root の capture フェーズ `capture_any_mouse_down`** と **GPUI フォーカス変化**の 2 つの合図から呼ぶ。PDF タブ（並行セッションが同日に追加）も同じ経路に載せた
- 学び/罠: **GPUI にマウスダウンが届いたこと自体が「クリックが WebView の外に落ちた」証拠**になる（WebView 上のクリックは OS 子ビューが食って GPUI には来ない）。だからページ内クリックとフォーカスの取り合いにならない。逆に「毎フレーム GPUI のフォーカスから導出して言い直す」設計は、ページ内クリック（AppKit が黙って first responder を移す・こちらからは観測できない）を次のフレームで奪い返すので不可。手元の持ち状態は OS にいつでも裏切られるため、`key_focus` は記録に留め、OS への要求は毎回言い直す（`makeFirstResponder` は同じ responder なら AppKit 側が即 YES を返す no-op）。`install_window_close_hook` と同じで、ここも「OS が真実を持ち、こちらは観測できない」層
- 検証: `cargo test -p workspace --lib` 60 green / `-p webview_view` 3 green。新規の gpui テストは**クリック経路だけを見るように組んだ**（先に 1 度クリックしてフォーカス追跡を落ち着かせないと「描画時のフォーカス変化」経路でも通ってしまう）＋ capture リスナーを潰すと落ちることを確認。gpui の TestWindow は raw window handle を持たず `build_as_child` が panic するので、生成だけ止める針（`disable_native_webviews_for_tests`）を webview_view に置いた。**実機の日本語入力は未確認**（IME は offscreen でも自動テストでも再現できない＝人の手番）
- 次: 実機で「HTML プレビュー表示 → composer をクリック → 日本語入力」。⌘⇧Enter（AI 全画面）で打てるのに通常配置で打てない、が直前の症状

## 2026-09-11 — PDF を OS のビューアでタブに開く / ACP の「選択肢つき質問」が出ない件
- やったこと: 報告2件。①**PDF が開けない**（クリックしても無反応）。原因は `.pdf` が画像判定に掛からずテキスト経路へ落ち、ropey の `Rope::from_reader` が invalid UTF-8 で失敗して `eprintln!` するだけだったこと。自前レンダラ（pdfium 等）は idle メモリ予算と配布サイズに効くので採らず、**HTML プレビュー用に既に在る `webview_view` に相乗り**して OS のビューア（macOS = WKWebView の PDFKit / Windows = WebView2）に `file://` を渡す方式にした。`crates/workspace/src/workspace/pdf_view.rs` を新設し `TabContent` に第3の具体型 `Pdf` を追加（Image と同じ「編集・保存・LSP・hot exit の対象外」）。可視性とキーボードフォーカスの同期は HTML プレビューと同じ経路に載せ（`sync_html_preview_visibility` → `sync_native_view_visibility` に改名し PDF も回す）、非表示 WebView の回収弁 `html_preview_evict_minutes` も共有。remote (SSH) は OS 子ビューがローカルパスしか読まないので、Host 経由で読んだバイト列を `<cache>/remote-pdf/` へ落として**その複製**を見せる（`CachedRemotePdf`。タブが所有し `Drop` で消す＝リモートの中身を手元に残さない。外部変更は画像タブと同じく背景で読み直して複製を上書き）。Linux（`is_supported()` false）だけがフォールバックカードで、「右クリック →『既定アプリで開く』」へ誘導する。②**エージェントの選択肢つき質問が UI に出ない**（AskUserQuestion に答えられない）。`claude-agent-acp` は AskUserQuestion を ACP の form elicitation に変換し、質問ごとに「選択欄 `question_<n>`」+「任意の自由入力 `question_<n>_custom`」の 2 プロパティを送る。necoder の `simplify_elicitation_form` は「全プロパティが単一選択でなければ非対応」なので、この Other 欄 1 つでフォームごと Decline → アダプタ側が `answers: {}` に畳んで「答えなかった」になっていた。`_meta._askUserQuestionCustomAnswer` 印の付いた選択肢無し string だけ読み飛ばすようにして解決。あわせて **multiSelect の質問（`type: array` + `items.anyOf` / `items.enum`）も対応**した: `ElicitationField.multi` を足し、応答は `ElicitationContentValue::StringArray` で返す。カードは押すたびにトグルし（単一選択と同じ「押した瞬間に送信」だと 1 つ目で確定してしまう）、確定は「これで回答」でのみ。Remote PWA も `<select multiple>` にし、`question_response` は文字列と配列の両方を受ける
- 学び/罠: **「OS の PDF viewer を使う」に必要な新規コードはほぼゼロだった** — `WebViewView::local_file` は HTML 専用の分岐を持たず任意の `file://` を読むので、器（タブの具体型 + フォーカスハンドル）だけが足りていなかった。第2の利用者が現れて初めて、この層が汎用だったと分かる。ACP 側の学び: **非対応フォームを丸ごと Decline する安全策は、任意フィールドが 1 つ混ざるだけで「質問が存在しないこと」に化ける**。しかも失敗はエージェント側で「ユーザーが答えなかった」に見えるので、こちらのログには何も出ない。`_meta` の共有印（ベンダ接頭辞なし＝Codex 等の他ブリッジも同じ印を使う取り決め）は、こういう「読み飛ばしてよい付属品」を伝える口として効く
- 検証: `cargo test --workspace --features necoder/runtime-shaders -- --test-threads=1` 56 バイナリ全 green。新規 5 本: `opening_a_pdf_gives_a_native_viewer_tab_not_a_text_buffer`（UTF-8 として読めない最小 PDF が Pdf タブになり、テキストバッファを作らない）/ `remote_pdf_is_cached_locally_and_removed_on_drop`（複製の中身・拡張子保持・上書き・drop で消える）/ `pdf_extension_is_matched_case_insensitively` / `ask_user_question_custom_answer_field_is_skipped_not_rejected`（実アダプタが送る形の schema で選択欄が残る・multiSelect も簡約できる・印無しの自由入力は従来どおり非対応）/ `multi_select_question_toggles_and_waits_for_submit` + `single_select_question_still_submits_on_click`。`relay` の node テスト 11 green。`cargo check -p necoder --features necoder/runtime-shaders` 緑。**PDF が実際に描かれるかは未確認** — ネイティブ子ビューは GPUI の offscreen キャプチャに写らず、このマシンは Metal 無し。実機の手番
- 学び（並列実行の罠）: `cargo test -p workspace` を並列で回すと GPUI の test scheduler が数本タイムアウトする（同じマシンで別セッションがビルド中だと再現しやすい）。落ちる顔ぶれが毎回変わるので実装の不具合と紛らわしい。`--test-threads=1` なら安定して全通過するので、赤を見たらまず直列で確かめる
- 次: 実機で ①`.pdf` をクリック → タブに PDF が出る ②別タブへ切替 → PDF が消える（OS 子ビューが残らない）③PDF タブから composer へ移って日本語が打てる（前エントリの first responder 経路）④エージェントの選択肢つき質問がカードで出て答えられる（単一・複数の両方）⑤remote (SSH) の PDF。残件: Other 欄は読み飛ばすだけで自由入力欄は出していない（答えたい時は「答えない」で畳んで composer に書く）。PDF のアイコンは `file-generic.svg` へのフォールバック（`file-pdf.svg` 未作成）

## 2026-09-11 — 出荷ビルドでは管制 IPC が一度も立っていなかった（QR が出ない正体）
- やったこと: 「QR が発行できない。しかも Node.js 22 の話が出る」の調査と根治。①**根本原因**: `main.rs` の `start_control_ipc` 呼び出しが**開発用 probe 群の `#[cfg(debug_assertions)]` ブロックの中**にあり、**release ビルドは `~/.necoder/gui.sock` を一度も bind していなかった**。Remote host は `remote_snapshot` を GUI へ IPC して snapshot を取るので、必ず `necoder_not_running_or_update_required` で pair に失敗する。`ne` CLI も GUI 不在と誤判定して LaunchServices 経由に落ちていた（開けてしまうので誰も気づかない）。窓ごとに 1 回起こす `start_control_ipc_for_window` を cfg の**外**（窓の初期化 update 内）へ出し、届かない時は 500ms×10 まで再試行する（`let _ =` の握り潰しも解消）。②**socket の自己修復**: 30 秒ごとに `control_ping`（accept スレッドが即答＝UI を経由しない新メソッド）で持ち主 pid を確かめ、自分でなければ張り直す。bind 失敗も AddrInUse 以外で return せず 15 秒ごとに再試行。③**失敗の言い分けをやめる**: 設定画面の QR エラーは原因に関係なく「Node.js 22以降・同梱・接続設定・ネット接続を確認」の固定文だった。host / CLI が返すコードを allowlist（`KNOWN_FAILURES` → `failure_message`）で分類し、原因ごとの一文にした（ja/en 各 +11 キー）。生の stderr は画面にもログにも出さない（ペアリング URL やパスが混ざり得るため）。`cli.rs` の 3 つの失敗に `node_missing` / `host_not_bundled` / `host_not_built`、`relay/host/cli.mjs` に `run_init_first` / `host_startup_failed` を付けた
- 学び/罠: **「出荷バイナリに文字列が無い」は最初の手がかりになったが、単独では根拠にならない**。`remote_snapshot` のような短いリテラルは release では memcmp に畳まれて消える。決め手になったのは「同じ release ビルドで `管制 IPC を開けない` も `control_ping` も消えるのに、settings の新キーは残る」= **ファイル単位で丸ごと落ちている**という比較。そこから cfg の囲いに辿り着いた。もう 1 つ: **`#[cfg(debug_assertions)]` の大きなブロック（600 行超）に常時動くべきコードを紛れ込ませない**。コメントには「常時」と書いてあったのに 8/30 以降ずっと嘘だった（`~/.necoder/gui.sock` の mtime が 8/30 のまま = 最後に debug ビルドを起動した日）
- 検証: `cargo test --workspace --features necoder/runtime-shaders` 56 バイナリ green（新規 4 本: control_ipc の ping 2 本 / settings の失敗分類 2 本）。**release ビルドで実機確認**: 隔離 `NECODER_HOME`/`NECODER_GUI_SOCK` で起動 → 4 秒で socket を張り `control_ping` と `remote_snapshot` が応答（修正前の release は無反応）→ socket を外から `rm` → **28 秒後に自動で張り直し**て再び応答。CLI の失敗コードは `run_init_first` が stderr に出ることを実行で確認。エラー文言そのものの目視（設定画面）は未 — 既存行の文字列差し替えでレイアウトは変えていない
- 次: ユーザーの `/Applications/necoder.app` を入れ替えて QR 発行を実機で通す（差し替えると稼働中のセッションが一度落ちる）。入れ替え後は `lsof -U -p <pid> | grep gui.sock` で持ち主が居ることを確認できる

## 2026-09-11 — Fleet の「作業」タブを机らしくする（3 段の作業ツリー・タブ行 1 本・縦タブ）
- やったこと: 別セッションが実装した `work_layout.rs` / `workbench.rs`（Fleet 中央タブ「作業」= リポジトリ 1 つを机として開き、worktree を列に並べる面）が**ドキュメント無し・目視未検証**で止まっていたので、目標画（ユーザーが描いた ASCII 画面）と突き合わせて残りを埋めた。①**作業ツリーを 3 段に**（リポジトリ → worktree → 面）。以前は「アクティブな worktree の Terminal だけ」3 段目が出ていた。いまは**列として見えている worktree は全部**、スレッド行（名前 + `activity_dot` + 承認待ちなどの一言）と `› Terminal N` を並べる。スレッド行クリックで `reveal(Agent)` + `focus_thread`＝**左のツリーがスレッド切替器**になる。リポジトリ行の ▾ で畳める（`work_layout.expanded` に保存） ②**ペイン内のタブ行を 1 本に**: 埋め込みの `AgentPanel` は自前のスレッドタブ行を描かない（`set_embedded`）。メタ行（状態・猫・トークン）も 1 行に詰める（猫は `MascotView::set_height(26)`＝親の器だけ縮めると絵が切り取られる）。**トークンメーターは畳まない**（常時可視は必須要件）。**猫は縮めすぎない** — 最初 26px にしたら「nyaco がめちゃくちゃ小さい」と指摘され 46px へ（necoder の顔で、状態を形と動きで伝える担当でもある。3 列でも表情が読める下限）。chrome は 1 列あたり約 160px → 約 115px。あわせて**埋め込み時は composer の基準高を最小（46px）に**した — 上下分割したペインでは背が低くなり、空の入力欄が 86px を占めると composer ごとはみ出して「停止 esc」や実行中の行が切れていた（実画面の目視で発見） ③**面の名前 = スレッド名**（「監督」「レビュー」…）。列見出しは 2 行にして 2 行目にいま映している面を ` / ` で連ねる＝**見出しだけで中身が読める** ④縦タブのとき識別色を**下線から左バーへ**（§1.3 の許可リスト内で向きだけ変える）⑤ペインの ⋯ に「新しいスレッド」 ⑥ドキュメント: UI-SPEC §6.1（面の仕様）/ ARCHITECTURE §7.4（`work_layout` は配置しか持たない・端末は名札で置く）/ GLOSSARY（作業・作業ツリー・列・ペイン・面・配置）/ ROADMAP の M14 に完了項目（M14 #201 の残 (b)「複数 space 同時表示」の回収）
- 学び/罠: ①**このマシンでも実アプリの offscreen 目視ができて、しかも文字が写る**（`--features necoder/runtime-shaders,necoder/screenshot` + `NECODER_SCREENSHOT`）。09-10 の「offscreen は文字がラスタライズされない」は `settings_preview` example の話で、本体の `render_to_image` 経路は font-kit を通るので別（メモにも書いてあった通り）。実 worktree 3 本の fixture（`git worktree add` した一時 repo）+ `NECODER_WORKBENCH_PROBE` で、目標画と 1 枚ずつ突き合わせられた ②**前のエージェントの実装は動いていた** — `cargo check` も 56 バイナリの test も緑で、止まっていたのは「目視」と「文書」。動くコードと**正である文書**は別の成果物で、CLAUDE.md の「実装と文書が乖離したら文書を直す方を先に」はこの順番のこと ③**面を切り替える入口が複数ある機能では、同期を呼び出し側に配らない**。`set_embedded` を最初は「面を出入りする場所」ごとに呼んでいたが、管制トグル（`toggle_control_center`）・管制からの没入（`immerse_from_control`）・AI 全画面（`toggle_agent_full_screen_state`）の 3 つを取りこぼしていた（＝作業面から抜けた後もスレッドタブ行が消えたままになる）。`Workspace::render` で 1 回だけ同期する形に変えた（既にある `sync_native_view_visibility` と同じ流儀。最後に適用した値を持って早期脱出する）。④一度「縦タブ全面で描画が 40 秒返ってこない」と見えたが、**前の起動の necoder が 3 本生き残っていただけ**（各自が実 PTY を持つ）。`pkill` して測り直すと 4 秒。**遅いと言う前に ps を見る**
- 検証: `cargo test --workspace --features necoder/runtime-shaders` 56 バイナリ green（i18n parity 含む）。`NECODER_WORKBENCH_PROBE` を実 worktree 3 本（main / feature/auth / feature/pay）で実行 → 独立した PTY 2 本・下ドック ⇄ 作業面の往復で Entity 同一・タブの向き切替・配置の保存往復が通り（`WORKBENCH_PROBE_OK`）、**横タブ / 縦タブの 2 枚を offscreen 目視**して目標画と一致を確認
- 次: 本人の実機で 3 列並走の体感（列幅をどこまで縮めて読めるか・縦タブは 150px を食うのでペイン単位の切替が要るかどうか）。残件: 作業ツリーの 3 段目に**ファイル面 / 変更(Diff)** の行が無い（いまはスレッドと Terminal だけ）・リポジトリを跨いだ列の並べ替えは未対応（机はリポジトリ単位で閉じている）

## 2026-09-11 — transcript の heredoc 畳み込みを script まで広げる / AI 全画面が他のタブを隠す
- やったこと: 報告 2 件。①**`python3 - <<'PY'` の script 全文が Step 見出しに素で出る**。`compact_shell_heredoc_write` は `cat > path <<EOF` だけを対象にしており、標準入力へ流し込む実行は明示的に対象外だった（テストにも `None` と書いてあった）。`CompactHeredoc::{Write, Script}` へ一般化し、Script は**走ったコマンドを見出しに残して本文だけ展開領域へ**回す（`python3 -` + 本文）。ついでに終端規則を shell に合わせ（最初に marker だけの行まで）、heredoc の**後続コマンドを見出しに残す**ようにした。後続がある `cat >` は「書き込み path」だと実行内容を取りこぼすので Script 側へ倒す ②**AI 全画面中に設定タブ・他のタブが開けない**。全画面は中央を丸ごと Agent に差し替えるので、`show_settings` も新しいタブも描画に出ない＝「開いたのに何も起きない」。`exit_agent_full_screen` を足し、エディタ領域を前に出す**対話操作**（`open_file` / 設定を開く ⌘, / レール ⚙ / diff の一時タブ）から呼ぶ
- 学び/罠: ②で「render 側で `show_settings` を優先する」形にしなかったのは、それだと設定しか直らずタブ全般が残るから。畳む側（状態を変える）に倒すと、閉じた後に全画面へ戻らない代わりに挙動が予測可能になる。呼ぶのは**対話経路だけ**に限る — `open_file_sync`（起動復元）から呼ぶと勝手に全画面が解ける。既存の「対話で開いたら設定ホームは退く」が同じ線引きで既に置かれていたので、そこへ並べた
- 検証: `cargo test -p agent_panel` 43 green（heredoc 4 本: cat 書き込み / stdin script / 後続コマンド / 終端なしは畳まない）、`cargo test -p workspace --lib` 62 green（新規 1 本: 全画面中に設定を開く・ファイルを開くと畳まれる）。`cargo check -p necoder --features necoder/runtime-shaders` 緑。**目視は未**（transcript の見た目は実アプリの手番）
- 次: 編隊モードにも同じ穴が残る（`editor_surface_visible` が fleet を外すので、編隊中にファイルを開いても中央に出ない）。設定は fleet_view が中央へ出す経路を持っているが、タブは持っていない。work surface の作業と一緒に見るのが早い

## 2026-09-11 — 上部に固定した「いまの問い」が長いと回答を押し出す
- やったこと: 報告「直前の入力が上に固定されるのは良いが、長いと ACP の出力が見えなくなる。自動で縮めてクリックで展開してほしい」。`render_pinned_prompt` は `truncate()` で 1 行のつもりだったが、**`truncate()` は横あふれしか畳まない** — 改行を含む問いはそのまま行数ぶん帯が伸びていた。畳みを `flatten_digest_line`（先頭行 + …）へ変更し、▸ で展開/折り畳み。展開しても `PINNED_PROMPT_MAX_HEIGHT`（160px）で頭打ちにし、あふれは帯の中でスクロール＝**帯が回答を潰さないことを構造で保証**。展開状態は transcript の User エントリと同じ `expanded_inputs`（`(thread.id, entry index)` 鍵）を共有する（同じ 1 発話を別々に持たない・新しい問いが来れば鍵が変わって自動で畳まれる）。▸ は `stop_propagation` で帯本体の「最新へジャンプ」を巻き込まない
- 学び/罠: **`truncate()` を「1 行にする」と読んではいけない**。畳むのは `text-overflow` 相当で、`\n` は素通りする。改行を含みうる文字列を 1 行枠へ置くなら、文字列側で潰す（`flatten_digest_line`）のが唯一確実。この罠は稼働中ライン・編隊セル・ビーコンで既に踏んでいて `flatten_digest_line` が在ったのに、固定帯だけ経由していなかった
- 検証: `cargo test -p agent_panel` 46 green（新規 1 本: 畳んだ側に改行が無い / 展開で全文 / 短い 1 行には ▸ を出さない）。`cargo check -p necoder --features necoder/runtime-shaders` 緑。**目視は未** — `screenshot-app.sh` は実アプリを起動して**画面全体**を撮る（作業中の画面に窓が出る）ので勝手には回していない。固定帯は「直近の問いが上端より上へ隠れている」状態でしか出ないため offscreen でも状態を作りにくい
- 次: 実機で長い問い（複数行）を投げて、帯が 1 行になること・▸ で開いて 160px で止まることを見る

## 2026-09-11 — 通知音を「完了」と「入力待ち」に分ける（同梱の合成音）

- やったこと: 余談の「完了音を猫の鳴き声にしたい」から。①**音源の作り方を決めた**（`docs/DECISIONS.md` §8）:
  CC0 の実録音を 5 本拾って加工（ピッチ +4〜7 半音・低域カット・0.5s に詰め）まで試したが、
  **生録音は通知音にならない**（部屋鳴り・ノイズ床・音程が無い・立ち上がりが鈍い）。加算合成へ切り替え、
  `scripts/gen-chime.py` で **`assets/sounds/done.wav`**（一声の「にゃー」0.45s）と
  **`waiting.wav`**（呼びかけの「にゃにゃっ」0.45s）を生成。猫を似せるのではなく**節回し**だけを
  音程に載せ、フォルマント 2 本（780/1900Hz）で声に寄せる。②**`crates/agent_panel/src/sound.rs` 新設**:
  wav は `include_bytes!` で埋め込み（マスコット PNG と同じ理由 — 実行時に `CARGO_MANIFEST_DIR` を
  読むとビルド機のパスが焼き込まれて配布物が鳴らない）。`afplay` はパスしか取らないので、初回だけ
  一時ディレクトリへ展開（同じ長さなら書き直さない・別名で書いて rename）してパスを使い回す。
  ③**場面を 2 つに分けた**: 完了（`TurnEnded` 成功）は従来どおり **window 非アクティブ時のみ**。
  入力待ち（`PermissionRequest` / `ElicitationRequest` で実際に止まったとき）は**そのスレッドが
  見えていないとき**＝別ウィンドウ **または同じ窓の別タブ**なら鳴らす（Blocked は*こちらの手番*で、
  裏のタブで止まったまま気づかないのが一番困る）。auto-allow で素通りした要求・ミュート中は鳴らさない。
  ④**設定**: `completion_sound`（bool）を廃し `sound_done` / `sound_waiting`（`"nya"` / `"system"` /
  `"off"` / **任意のファイルパス**・`~/` 可）へ。設定画面はセグメント 2 行、i18n ja/en 5 キー追加。
- 学び/罠: ①**「合成」で失敗する領域と得意な領域は別**。生き物の声を数式で似せるのは筋が悪いが、
  ベル（倍音を並べて指数減衰）と節回し（周波数の軌跡）は合成が得意な側で、録音を加工するより早く良くなった。
  ②**設定画面の offscreen 検証は縦に詰まる**: `settings_preview` の窓高さは OS が画面高（1041pt）で
  クランプするので、`NECODER_SETTINGS_PREVIEW_HEIGHT` を上げても下のセクションは写らない。
  今回は `body` から上のセクションを一時的に外して撮り、確認後に戻した（セグメントの選択位置が
  seed 値どおりであることを目視）。恒久策が要るなら preview 例にセクション選択の env を足すのが筋。
  ③`settings_core` の `Settings` は `serde_json::from_value` で**全体が一度に**デシリアライズされるので、
  既存キーの**型を変える**と古い settings.json を持つ人の設定が丸ごと落ちる。今回は互換層を足さず
  **キー名を変える**ことで回避した（未知のキーは無視される・旧版の救済はしない方針と両立）。
...
## 2026-09-14 — v0.1.17 リリースと、CI の WebKit でだけ落ちた QR スキャンのテスト
- やったこと: 未コミットだった 5 セッション分を 3 本の topical commit に畳んで push し、**v0.1.17 をリリース**
  （CHANGELOG 0.1.17 → `Cargo.toml` 0.1.17 → タグ。署名・公証済み `necoder.dmg` と
  `necoder-windows-x64.zip` が Releases に付いた）。`cargo test --workspace` / `cargo deny check` /
  relay `npm test` 17 pass / `tsc --noEmit` すべて green
- **CI の WebKit だけで新しい QR スキャンの browser test 2 本が落ちた**。手元（chromium + webkit 8 pass）
  との差が分からず、原因に辿り着くまで 3 往復かかった:
  - **`addInitScript` で `navigator` を差し替えても、WebKit では消える**。文書が確定する時に navigator を
    作り直すので、init script が載せた own property は残らない。`window` に置いた印は残るため
    「差し込めた」ように見えて、実際には**本物の `getUserMedia` が呼ばれる**
  - 本物は permission 待ちのまま**解決も拒否もしない**ので、失敗が 45 秒のテストタイムアウトに化けて
    原因が読めない。手元の Mac は内蔵カメラがあるので素の代入が通ってしまい、この差が CI でしか出なかった
  - 直し: **`page.goto` の後に `page.evaluate` で差し込む**（アプリがカメラを触るのはボタンを押した後なので
    文書ができてからで足りる）。あわせて `expectFakeCamera` で**印が付いているか＋実際に呼ばれるか**を
    3 秒で打ち切って確かめ、落ちた時は形（own property か・`mediaDevices` は毎回同じ物か）を報告する
  - 途中で**製品側の実バグを 1 つ拾った**: カメラ拒否の catch が後始末（トラック停止・`dialog.close`）を
    先にやってから文言を出していたので、後始末が環境依存で転ぶと**黙って何も起きないボタン**になる。
    文言を先に出し、後始末の失敗は見せないようにした
- 学び/罠:
  - **「手元で通るのに CI で落ちる」の第一容疑者はハードウェアの有無**。カメラ・マイク・GPU に触る
    テストは、無い機械での挙動（API ごと生えない / permission が永久に返らない）を先に疑う
  - **失敗がタイムアウトに化けるテストは、原因を報告するテストに書き換える**。`Promise.race` で
    打ち切って状態を JSON で吐くようにしたら、次の 1 往復で答えが出た
  - 診断のために `await element.onclick()` を直接叩くのは**やめた方がいい** — ハンドラを二重に走らせると
    `page.evaluate` ごと固まって、診断が新しい謎を生む
- 次: PWA（`relay/public`）の直しは Worker 側のデプロイで初めて反映される。`.app` に同梱されるのは
  `relay/dist/host.mjs` だけなので v0.1.17 の成果物には含まれない

## 2026-09-14 — 通知音が「犬」だった話（机上で2回外し、候補を聴いて決めた）
- やったこと:
  - **ユーザー報告「終了の音が猫っぽくない」からの作り直し**。合成（`scripts/gen-chime.py`）に
    **声道モデル**を入れ、「にゃ」を口の開閉として鳴らすようにした: 閉じた鼻音 [ɲ] → 開いた [あ] → 閉じ。
    キーフレームは `(位置, F0, (F1,F2,F3), 口の開き)`。口を閉じている区間は高い倍音を指数で殺す。
  - **ここで2回外した**。①初版は**フォルマント固定**（音程の軌跡だけ猫の真似）②改訂版は母音を聞かせようと
    **F0 を 370-640Hz へ下げた** — これが決定打で、ユーザーの評は「犬っぽい」。低い F0 + 平らな持続 +
    鼻音の長い減衰は**犬のくぅーんそのもの**だった。
  - **やり方を変えた**。机上で当てにいくのをやめ、`--candidates` を足して**候補を8本ずつ /tmp に書き出し、
    耳で選んでもらう**形にした。3周で決着:
    1周目（8本）→「2（F0 780-1080）と 6（子猫チャープ）系統。やっぱり高い方がいい」
    2周目（高さの階段8本）→「1（= 2 を 1.2 倍・F0 940-1300Hz）がいい」
    3周目（長さだけ8本）→ 完了 **450ms の一声**・入力待ち **250ms の二声**
  - **同梱は3声**（すべて同じ声・尺違い + チャープ）: `nyaan`（にゃー・**既定**。完了 450ms / 待ち 250ms×2）/
    `nya`（にゃっ・250ms / 180ms×2）/ `mew`（みゃっ・子猫のチャープ 200ms / 150ms×2）。
    低い F0 で作った旧 `trill`（クルル）は**捨てた** — 誰も聴いて選んでいない音を同梱しないため。
  - **設定は押すとその場で鳴る**（試聴）。`SettingsViewEvent::PreviewSound` を足して shell（chrome.rs）で
    `agent_panel::sound::play` を呼ぶ。設定 crate は agent_panel を知らないままにできる。
  - 選べる値の正は `settings_core::SOUND_VOICES` に1本化（設定画面と `sound.rs` の両方がこれを見る）。
  - `sound.rs`: 展開（ファイル書き込み）を**再生スレッドの中へ移した**。声が増えて OnceLock のキャッシュが
    (声 × 場面) になるところで、そもそも UI スレッドで I/O する理由が無かったと気づいたため。
- 学び/罠:
  - **猫と犬を分けているのは音程の高さ**（猫 900-1300Hz / 下げると犬のくぅーん）。「母音を聞かせたいから
    F0 を下げる」は音声学的には正しいが、**種の同定を壊す方向**だった。トレードオフの向きを間違えた。
  - 他の3つも効く: 「にゃ」の入りは **F2 が高いところから落ちる**（[ɲ] = 口蓋。上げると「まおー」）/
    **平らな持続を作らない**（滑らかに伸びる音は犬）/ **ざらつき**（不規則 AM。サイン波の足し算はツルツル）。
  - **自分が知覚できない出力は、机上で詰めてはいけない**。スペクトログラムで「倍音が動いているか」までは
    検証できるが、「猫か犬か」は判定できない。**候補を並べて人に選ばせる形に早く切り替えるべきだった** —
    1本ずつ出して感想をもらう loop を2周やったのは、ユーザーの時間の使い方として悪い。
    （同じ形の問題: 色・フォント・文言のニュアンス。**評価関数が人間の中にあるもの全般**）
  - 候補は**一軸だけ振る**と決着が速い（高さ → 長さ、の順に潰した）。混ぜて8本出した1周目は
...
- 続き（同日・push 後）: Windows でテストが**初めて走った**結果、`transcript_links_only_cover_what_resolves` が落ちた。`ui::links::split_position` が `C:\Users\…\ROADMAP.md:157` を `rsplitn(3, ':')` に掛けるとき、頭の枠を**ドライブ文字 `C` に食われて** `path:line` の腕に入らず行番号が消えていた。ドライブ接頭辞（`drive_prefix_length`）を先に切り離してから位置を解釈するよう分解し、回帰テスト `a_drive_letter_is_not_mistaken_for_a_position` を追加
- `fleet_panes_keep_their_threads_and_remote_targets` の非決定性も直した（mac・Windows の両方で落ちていた。`Detected activity on thread Some("PTY reader") … Your test is not deterministic.`）。`add_terminal_to_selected_task` → `TerminalDock::start_session` が**実シェルを起動**し、その PTY リーダースレッドが `gpui::test` のスケジューラに割り込む。既にあった `TerminalView::new_test` / `ensure_active_test` と同じ考え方で、Dock に `use_test_terminals()`（test-support のみ・以後 `create_terminal` が PTY 無しの端末を作る）を足し、テストの各 session で立てた。3efcbef でも同じ落ち方をしていた既存の穴で、今回の削除とは無関係
- 次: M16 C0（変更なし）

## 2026-09-19 — Chat モードを実装（M16 C0〜C6・`feature/chat-mode`）
- やったこと: ブランチは **git worktree**（`../necoder-chat`）で切った — main の作業ディレクトリは別セッションも使っていて、同じ場所で checkout すると相手の足元のブランチが変わるため。新 crate **`chat_core`**（GPUI を知らない: プロンプトとプリセット / 書類フォルダの `necoder/<日付 最初の文>/` の命名と寿命 / 権限の裁定 / ローカル日付）+ `acp_client::preset`（`_meta` を `session/new` と `session/load` の両方へ）+ `paths::documents_dir` + `storage` の `thread_chats` / `thread_chat_paths` / `search_turns` + `agent_panel/src/chat.rs`（Chat モードのパネル）+ `workspace/chat_view.rs`（面）+ `webview_view::sandbox`（成果物の隔離）+ 画像つき prompt（M12 の未了項目を同時に消化）。詳細は `CHAT.md` §7・`UI-SPEC.md` §13・ROADMAP M16
- 検証: `RUSTFLAGS="-D warnings" cargo check --workspace --all-targets` 緑・`cargo test --workspace` 緑（新規: chat_core 24 / agent_panel chat 10 / workspace 6 / webview sandbox 7 / acp_client 6 / storage 2 / paths 5）。`cargo check -p chat_core --target x86_64-pc-windows-msvc` 緑。**実エージェント**: `probe_chat_preset` 11 項目・`--image`・`--compare`。**GUI**: データ・ソケット・書類フォルダを全部隔離した offscreen 起動で、見本（`seed`）と実エージェント（`send:`）と**再起動後の再開**（`pick` → `send:`）を通した
- 学び/罠: ①**claude.ai のコネクタが漏れ込む**（`settingSources: []` でも `strictMcpConfig` でも）。文脈が最初のターンから 11 万トークン。`ENABLE_CLAUDEAI_MCP_SERVERS=false` で 5,202。**実エージェントに「使える道具を全部書け」と聞く 1 ターン**が無ければ気づけなかった（行動は正しく見えていた）②**`PanelEvent::FilesTouched` は書かれる前に届く**（許可リクエストの時点）。そこで `is_file()` を見ても無い＝成果物が右に開かない。覚えておいてターン終了時に取り込む ③**ACP の権限の応答は並びの添字**。選択肢を `retain` で間引くと別の選択肢を選んだことになる（出さない判断は描画側で）④**「常に許可」をエージェントへ返さない** — 相手が自分のモードを切り替えて、以後のリクエストがこちらへ来なくなる ⑤`Workspace` は Deref でアクティブな session を指すので、Chat を別持ちにして Deref を切り替えると**開く・タブ・プレビューが全部そのまま動く**。代わりに `projects.get_mut(active)`（枠への書き戻し 14 か所）と `active_worktree()`（host を引く 4 か所）が Chat で黙って外れる/素通りするので、`slot_mut` / `active_host` に寄せた。後者は「タブが開かない」テストで見つかった ⑥OS の子ビュー（WebView）は裏に回った session の分も**明示的に隠す**（Chat ⇄ プロジェクトで、裏のプレビューが浮いて残る）⑦offscreen の transcript が薄く写るのは初期コミットからあるフェードインの 1 フレーム目（実機では起きない）。撮影用の設定に `reduce_motion` ⑧撮影は DB を開かない作りだったので、`NECODER_HOME` で隔離した時だけ開くようにした（永続化を含む機能を offscreen で検証できる）⑨`every_icon_file_is_registered` が、足したアイコンの登録漏れ（レールに出ない）を拾った ⑩`cargo fmt --all` は HEAD で未整形だった無関係のファイルも直す。触っていないファイルは `git checkout --` で戻してからコミットした ⑪wry はページに `window.ipc` の殻を常に置くが、ネイティブの受け口は `ipc_handler` を渡した時だけ登録される（「IPC が無い」の根拠はこちら）
- 次: **人の手番** = ①実機で Chat を開いて HTML プレビューの再読込を目視（C1）②`.md` を実際にドロップ → 確認 → 差分 → 巻き戻し（C3）③スクリーンショットの ⌘V（C4）④初回の書類フォルダの TCC ダイアログ。実装の残りは C5 の 5 件（一覧の幅・最後のチャットの復元・transcript 内検索・コードブロック単位のコピー・設定画面への露出）と、release ビルドでのメモリ再計測

## 2026-09-19 — Chat の残件（C5）を全部実装し、エージェントの重さに手を入れた
- やったこと: 前エントリで「後続」にしていた C5 の 5 件を全部実装（**一覧の幅の保存**＝左ドックと共有・右縁ドラッグ / **最後に開いていたチャットの復元** / **transcript 内検索**（`agent_panel/src/search.rs`・⌘F・全スレッド共通）/ **コードブロック単位のコピー** / **設定画面への露出**（「動作とエディタ > CHAT」と「MCP サーバ」））。あわせて実測から 3 つ: **全スレッド共通のエージェント自動停止**（`agent_panel/src/idle.rs`・`agent_idle_stop_minutes` 既定 15）・**npx キャッシュの直接起動**（`npm exec` のラッパーを持たない）・**claude.ai のコネクタの設定**（`claude_ai_connectors`）
- 検証: `RUSTFLAGS="-D warnings" cargo check --workspace --all-targets` 緑・`cargo test --workspace` 517 本緑。offscreen で検索バー（`find:<語>`）と設定画面（`settings:prefs` / `settings:mcp`）を目視。実エージェントで npx ラッパー無しの起動を確認（`launch: ~/.npm/_npx/<hash>/node_modules/.bin/claude-agent-acp []`）
- 学び/罠: ①**「重い」の正体を測り直したら Chat の話ではなかった** — エージェント 1 本 350〜700MB × 生きている数。稼働中の窓で 7.8GB（本体は 291MB）。Chat を作ったから増えたのではなく、**元から Editor / Fleet も同じ**だった。Chat 専用に入れた停止の弁を全スレッドへ一般化した ②**止めてよいのは「会話を引き継げるスレッド」だけ**。`loadSession` を広告しないエージェントを止めると文脈が失われる＝イベント（`SessionStarted`）に `resumable` を載せて UI が判断できるようにした ③`npx -y` は `npm exec` の親プロセスを残す（48MB/本）。キャッシュを直接起こせば消えるが、**版が一致する時だけ**にしないと更新が止まる ④claude.ai のコネクタは Chat 固有の問題ではなかった（Editor でも 6.7k トークン・2.4 秒の差）。前エントリで Chat だけ塞いだのは対症療法で、設定として出すのが正しかった ⑤検索の強調は `push_selectable_with_links` の 1 か所（transcript の文字が全部通る）に足すと、本文・コード・ツール出力・差分のどこに在っても同じように光る。**キャッシュ鍵の計算より前**に混ぜること（後だと検索語を変えても再描画されない）⑥大文字小文字を無視するのに `to_lowercase()` を使うと**バイト長が変わって強調範囲がずれる**（`İ` 等）。ASCII だけ畳めば長さは不変で、日本語には大文字小文字が無いので実用上これで足りる。加えて多バイト文字の途中に当たる偽の一致を `is_char_boundary` で弾く ⑦GPUI の `EditorView::plain` を行に置くときは**高さを明示する**（無いと潰れて文字が出ない。チャットの一覧の検索欄も同じ作法だった）
- 次: **人の手番**（前エントリと同じ 4 件）= 実機での HTML プレビューの再読込・`.md` の D&D → 確認 → 差分 → 巻き戻し・スクリーンショットの ⌘V・書類フォルダの TCC ダイアログ。実装側の残りは release ビルドでのメモリ再計測（`scripts/memory-usage.sh` の流儀）

## 2026-09-19 — Chat を main へマージし、「人の手番」だと思っていた 3 件を offscreen で通した
- やったこと: `feature/chat-mode` を main へマージ（`2b1b139`・別セッションの 2 コミットと競合なし）。そのあと、前エントリで人の手番に回していた 4 件のうち **3 件を offscreen で検証**して ROADMAP の C1 / C3 / C4 にチェックを入れた。新しいプローブ: `attach:<path>` / `approve[:always|reject]` / `rollback[:first]` / `paste-image` / `edit:<文字>`。ついでに見つけた実バグ 1 件（添付チップに長い絶対パスが出る）を直した（短い名前 + ツールチップ・貼り付けた画像は「貼り付けた画像」）
- 検証: **① プレビューの再読込** — 検証用ページが `document.title` に自分の中身を書き、necoder 側で拾う。実物の WKWebView で `BEFORE-EDIT` → エージェントの修正 → `AFTER-EDIT`。**② 渡したファイルの修正と巻き戻し** — 実エージェントにチャットのフォルダの外の `.md` の誤字 3 つを直させ、承認は 1 度だけ（「このチャットでは以後許可」）、最初のチェックポイントへ戻すと元に戻った。**③ 画像の ⌘V** — 実 PNG をクリップボードへ置いて `EditorView::paste` から添付まで。`cargo test --workspace` 519 本緑
...
- やったこと: push の権限が直り、1 本にまとめて **#30**（base `parity/integration`）を作った。CI の赤を順に
  ...
  **statusbar の項目の出し入れ**（右クリック・`statusbar_hidden`・O27）・**書体を選ぶ画面**（設定 › 外観・O27）・**シェルを選ぶ画面**（O25）・**SSH の接続テスト**（O37）・端末の **⌘T / ⌘W**（O24）・Task の**詳細**（レールの右クリック「詳細…」・O21 / A12）・エージェントの**使う / 使わない**（`disabled_agents`・O16 / B02）。
- 学び/罠:
