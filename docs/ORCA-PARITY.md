...
| O29（一部） | 同上 | #30 | CSV / TSV の表（⌘⇧V・RFC 4180・見出しの固定・行番号・仮想リスト）・画像のドロップ（Markdown に落とすと隣へコピーして落とした所に `![]()`・ほかのファイルはタブで開く）・front matter（プロパティの表）・目次（`[toc]`・押すと見出しへ）・横並びのライブプレビュー（⌘K V）。スラッシュメニュー・Mermaid・Markdown の注記は未 |
...
- **計画の続き**: O5（手元の Ports）・O21 の一部（外部の worktree・絞り込み・並べ替え・コピー・ピンの保存・休ませる・複数選択）・O26（エディタの所作・全部）・
  ...
  O19 の一部（統合で競合したら Task のエージェントに直させる）・O29 の一部（CSV / TSV の表・画像のドロップ・front matter・目次・横並びのプレビュー）・O37 の一部（リモートのダウンロード / アップロード・繋がらない理由の案内）。O15・O18 は Mac の worktree に途中があるので触っていない（push されれば取り込める）。
