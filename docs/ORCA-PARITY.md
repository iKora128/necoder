...
| O21（一部） | 同上 | なし | 外部の worktree・取り込み・統合先の取り違え・消えた worktree・Task の絞り込み・パス / ブランチ名のコピー。ピンの保存・手動の休眠・複数選択・並べ替え・詳細・親子は未 |
| O26 | 同上 | なし | 済: `path:行`・全部閉じる・外部アプリ・自動保存・プレビュータブ・ピン留め。実機未確認 |
| O20（一部） | 同上 | なし | ブランチ名・起点・既にあるブランチ・`.worktreeinclude`・準備の skip（＋ Task の詳細）・準備が失敗した Task の「準備をやり直す」/「準備を飛ばして始める」。絵文字名・repo ごとの base・sparse・進捗 / キャンセル・どこで動かすかは未 |
...
| O22 | 同上 | なし | リソース（メモリをプロジェクトごと・使っていないエージェントを止める）・片付け（まとめて終了 / 失うものが無い worktree をまとめて削除）。サイズでの並べ替えは未 |
| O16（一部） | 同上 | なし | repo ごとのレシピ（`.necoder/recipes/*.md` を `/` 補完へ）。エージェントの有効 / 無効・権限の既定・起動引数の UI・エージェント別の新規スレッドのキーは未 |
...
| O17（一部） | 同上 | なし | 質問カードの自由入力（Other 欄に書いて答える・選んだ案に添える。スマホの質問も）・サブエージェントの手順を親の Task の下に畳む。message rail・ツールのまとめ表示と個別停止・Codex の goal は未 |
| O27（一部） | 同上 | なし | 設定の検索（全ページから一致した行を集める・キー / 選択肢の名前でも当たる）。キー割り当ての GUI・UI ズーム・UI フォント・density・言語の切替・statusbar の項目は未 |
...
- **計画の続き**: O5（手元の Ports）・O21 の一部（外部の worktree・絞り込み・コピー）・O26（エディタの所作・全部）・
  O20 の一部（ブランチ名・起点・`.worktreeinclude`）・O13 の一部（スリープ抑止・通知の履歴・音量・スマホへの完了 push）・
  O22（リソース・片付け）・O23 の一部（fan-out）・O14（アカウント切替）・O16 の一部（レシピ）・
  O17 の一部（質問カードの自由入力・サブエージェントの手順）・O27 の一部（設定の検索）。O15・O18 は Mac の worktree に途中があるので触っていない（push されれば取り込める）。
...
| B27 | Codex goal | 無 | O2・O17 |
| B28 | エージェント別の新規タブのキー | 一部 | O16 |
