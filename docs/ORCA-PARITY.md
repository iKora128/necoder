...
| O20（一部） | 同上 | #30 | ブランチ名・起点・既にあるブランチ・`.worktreeinclude`・準備の skip（＋ Task の詳細）・準備が失敗した Task の「準備をやり直す」/「準備を飛ばして始める」・repo ごとの base（`task_base`）・作成中の行（段の表示・取り消し・やり直し）。絵文字名・sparse・どこで動かすかは未 |
...
| O25（一部） | 同上 | #30 | ターミナルの文字の大きさ・フォント・遡れる行数・カーソルの形（設定と設定画面・開いている端末にもその場で効く）。テーマの取り込み・シェルと引数・Quick Commands・OSC 133・TUI の状態推定は未 |
...
- **計画の続き**: O5（手元の Ports）・O21 の一部（外部の worktree・絞り込み・並べ替え・コピー・ピンの保存・休ませる）・O26（エディタの所作・全部）・
  ...
  O25 の一部（ターミナルの文字の大きさ・フォント・遡れる行数・カーソルの形）。O15・O18 は Mac の worktree に途中があるので触っていない（push されれば取り込める）。
