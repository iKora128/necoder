...
| O20（一部） | 同上 | #30 | ブランチ名・起点・既にあるブランチ・`.worktreeinclude`・準備の skip（＋ Task の詳細）・準備が失敗した Task の「準備をやり直す」/「準備を飛ばして始める」・repo ごとの base（`task_base`）・作成中の行（段の表示・取り消し・やり直し）・sparse checkout（`task_sparse`・cone）・絵文字の名前（`:rocket:` → 🚀）。どこで動かすかは未 |
...
| O16 | 同上 | #30 | repo ごとのレシピ（`.necoder/recipes/*.md` を `/` 補完へ）・エージェント別の新規スレッド（`workspace::NewThreadCodex` 等・パレットと keymap.json）・エージェントの有効 / 無効（設定の各行のスイッチ・選択肢から外しログインの確かめも起こさない）・権限の既定（エージェントの既定 / 聞かずに進める・選ぶとピルの記憶を一括で消す・記憶を 1 つずつ忘れる）・起動の上書きの画面（環境変数を足す / 自分のコマンド・引数 / 既定に戻す）。実エージェントでの確認は未 |
| O14 | 同上 | #30 | アカウントのフォルダ・切替（`agent_servers.<id>.env`）・公式 CLI でログイン。mac 実機未確認・Windows は未 |
| O23 | 同上 | #30 | fan-out（1 つの依頼をエージェントごとの Task へ・舞台に並べて比べる）・自動命名ブランチの改名（依頼から名前が作れなかった `task/task` を、最初のスレッドの名前か一発生成の英語の句から `task/<slug>` へ・push 前だけ）。実エージェントでの一発生成は未確認 |
...
| O27（一部） | 同上 | #30 | 設定の検索（全ページから一致した行を集める・キー / 選択肢の名前でも当たる）・表示言語の切替（その場で）・UI とコードの書体（`ui_font_family` / `code_font_family`・settings.json・直書きしていた 30 か所を `ui::ui_font` / `ui::code_font` に）。・statusbar の項目の出し入れ（右クリック・`statusbar_hidden`）・書体を選ぶ画面（設定 › 外観・入っている書体のピッカー）。キー割り当ての画面（⌘K ⌘S の一覧で押して変える・他が使うキーは置き換えるかを聞く・↺ で既定へ・keymap.json から消した束が再起動まで残る不具合も直した）。行の間隔（`density`・リストの行の高さ）。UI ズームは未 |
| O24（一部） | 同上 | #30 | 端末タブの改名（ダブルクリック・スレッドと同じダイアログ・空でシェルのタイトルに戻る）・⌘T / ⌘W（端末の中でドックのタブを開く / 閉じる）。横の分割（タブの中で 4 つまで・⌘\\・⌘W は前にいる端末だけ）。入れ子の分割・ドラッグで分割・エディタ領域の端末タブ・フローティングは未 |
| O37 | 同上 | #30 | リモートのダウンロード / アップロード（G09・Finder からエクスプローラへ落とす / 右クリックの「ダウンロード…」「このフォルダへアップロード…」・上書きしない・背景で送り、終わりの知らせから Finder で見せる）・繋がらない理由の案内（G02 / G04・OpenSSH のログを `-E` で読み、指紋・鍵・名前・拒否・応答なし・権限・暗号を見分けて次にすることを出す・パスフレーズの問いに `ssh-add` の案内）・接続テスト（パレット・`ssh host true` を 1 回）。接続先の登録（ピッカーの「＋ 接続先を登録…」・~/.ssh/config の末尾に足すだけ・同じ名前と書けない値は断る・登録したら接続テスト）。登録画面の後も ~/.ssh/config を正にしている）。SSH 実機未確認 |
...
- **計画の続き**: O5（手元の Ports）・O21 の一部（外部の worktree・絞り込み・並べ替え・コピー・ピンの保存・休ませる・複数選択）・O26（エディタの所作・全部）・
  ...
  O17 の一部（質問カードの自由入力・サブエージェントの手順・目標の操作・message rail）・O27 の一部（設定の検索・表示言語・書体・statusbar の項目）・
  O25 の一部（ターミナルの文字の大きさ・フォント・遡れる行数・カーソルの形・シェルと引数・Quick Commands・配色の取り込み）・
  O19 の一部（統合で競合したら Task のエージェントに直させる）・O29 の一部（CSV / TSV の表・画像のドロップ・front matter・目次・横並びのプレビュー・スラッシュメニュー）・O24 の一部（端末タブの改名・⌘T / ⌘W）・O37 の一部（リモートのダウンロード / アップロード・繋がらない理由の案内）。O15・O18 は Mac の worktree に途中があるので触っていない（push されれば取り込める）。
...
| A08 | 絵文字の名前（:rocket:） | 有（#30・Task / スレッド / 端末タブの名前の `:rocket:` を 🚀 に・よく使う 130 ほど） | O20 |
| A09 | ピン留め | 一部 | O21 |
| A10 | Sleep / Archive / Delete | 一部 | O21 |
| A11 | 複数選択の一括操作 | 無 | O21 |
| A12 | インライン改名・詳細ダイアログ | 有（#30・右クリックの「詳細…」） | O21 |
...
| A22 | 状態別ボード | 無 | §6 |
| A23 | 自動命名ブランチを作業内容から改名 | 有（#30・依頼から名前が作れなかった Task を、最初のスレッドの名前から改名・push 前だけ） | O23 |
...
| B07 | 権限の既定（Yolo / Manual 一括）・起動引数の上書き | 有（#30・権限の既定の一括とピルの記憶を忘れる・起動の上書きの画面（環境変数 / 自分のコマンド / 既定に戻す）） | O16 |
| B08 | hibernation | 有 | 済 |
...
| C02 | 端末の分割（無限・ネスト） | 一部（#30・タブの中で横に 4 つまで・⌘\\。入れ子・縦は無し） | O24 |
...
| C15 | インライン画像 | 無 | O45 |
| C16 | Windows のシェル選択 | 有（#30・設定の「ターミナルのシェル」・PATH の pwsh / powershell / cmd / bash / wsl） | O25 |
| C17 | 端末へのファイルドロップ | 無 | O3 |
| C18 | 端末のショートカット（⌘T・⌘⇧\ 等） | 一部（#30・⌘T / ⌘W でドックのタブ。分割の ⌘⇧\ は分割と一緒に） | O24 |
| C19 | 端末タブの改名 | 有（#30・ダブルクリック・空で元に戻る。保存はしない） | O3・O24 |
| C20 | シェル統合（OSC 133） | 無 | O25 |
| C21 | 実行中の端末を閉じる前の確認 | 無 | O4 |
| C22 | シェルの引数・既定のシェル | 有（#30・`terminal_shell` / `terminal_shell_args`・選ぶ画面は引数を除く） | O25 |
...
| D14 | フォント・UI ズーム・density | 一部（#30・書体と density（行の間隔）は有。UI ズームは未） | O27 |
...
| D22 | 設定の検索 | 無 | O27（済） |
| D23 | キー割り当ての GUI | 有（#30・⌘K ⌘S の一覧で押して変える・衝突を聞く・↺ で戻す） | O27 |
| D24 | 言語機能 | 有（優位） | 済 |
| D25 | 大規模での性能 | 一部 | O28 |
| D26 | 言語の幅 | 一部 | O30 |
...
| ID | Orca の機能 | 調査の判定 | 行き先 |
|---|---|---|---|
| G01 | SSH ターゲットの登録 UI・接続テスト | 有（#30・ピッカーの「＋ 接続先を登録…」で ~/.ssh/config に足す・接続テストはパレットから。登録は ~/.ssh/config のまま） | O37 |
...
| H08 | 設定の検索 | 無 | O27（済） |
| H09 | キー割り当ての UI | 有（#30・D23 と同じ） | O27 |
| H10 | UI ズーム・フォント・density | 一部（#30・D14 と同じ） | O27 |
