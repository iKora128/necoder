...
| O16（一部） | 同上 | #30 | repo ごとのレシピ（`.necoder/recipes/*.md` を `/` 補完へ）・エージェント別の新規スレッド（`workspace::NewThreadCodex` 等・パレットと keymap.json）・エージェントの有効 / 無効（設定の各行のスイッチ・選択肢から外しログインの確かめも起こさない）・権限の既定（エージェントの既定 / 聞かずに進める・選ぶとピルの記憶を一括で消す・記憶を 1 つずつ忘れる）。起動引数の UI は未 |
...
| B07 | 権限の既定（Yolo / Manual 一括）・起動引数の上書き | 一部（#30・権限の既定の一括とピルの記憶を忘れる。起動引数の画面は未） | O16 |
| B08 | hibernation | 有 | 済 |
