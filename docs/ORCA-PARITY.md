...
| O37（一部） | 同上 | #30 | リモートのダウンロード / アップロード（G09・Finder からエクスプローラへ落とす / 右クリックの「ダウンロード…」「このフォルダへアップロード…」・上書きしない・背景で送り、終わりの知らせから Finder で見せる）。接続先の登録画面・接続テスト・ホスト鍵とパスフレーズの案内は未。SSH 実機未確認 |
...
- **計画の続き**: O5（手元の Ports）・O21 の一部（外部の worktree・絞り込み・並べ替え・コピー・ピンの保存・休ませる・複数選択）・O26（エディタの所作・全部）・
  ...
  O19 の一部（統合で競合したら Task のエージェントに直させる）・O29 の一部（CSV / TSV の表）・O37 の一部（リモートのダウンロード / アップロード）。O15・O18 は Mac の worktree に途中があるので触っていない（push されれば取り込める）。
