...
| O13（一部） | 同上 | なし | 作業中はスリープさせない・通知の履歴（ベル）・通知音の大きさ。スマホへの完了 push・メニューバー・TCC の案内・ニュースのクリック移動は未 |
| O22（一部） | 同上 | なし | リソース（メモリをプロジェクトごと・使っていないエージェントを止める）。片付け画面（一括削除）は未 |
...
- **計画の続き**: O5（手元の Ports）・O21 の一部（外部の worktree）・O26（エディタの所作・全部）・
  ...
  O22 の一部（リソース）・O23 の一部（fan-out）・O14（アカウント切替）。O15・O18 は Mac の worktree に途中があるので触っていない（push されれば取り込める）。
