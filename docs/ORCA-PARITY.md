...
| O24（一部） | 同上 | #30 | 端末タブの改名（ダブルクリック・スレッドと同じダイアログ・空でシェルのタイトルに戻る）。分割・ドラッグで分割・エディタ領域の端末タブ・フローティング・キーは未 |
...
- **計画の続き**: O5（手元の Ports）・O21 の一部（外部の worktree・絞り込み・並べ替え・コピー・ピンの保存・休ませる・複数選択）・O26（エディタの所作・全部）・
  ...
  O19 の一部（統合で競合したら Task のエージェントに直させる）・O29 の一部（CSV / TSV の表・画像のドロップ・front matter・目次・横並びのプレビュー・スラッシュメニュー）・O24 の一部（端末タブの改名）・O37 の一部（リモートのダウンロード / アップロード・繋がらない理由の案内）。O15・O18 は Mac の worktree に途中があるので触っていない（push されれば取り込める）。
