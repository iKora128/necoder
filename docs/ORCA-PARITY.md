...
| O20（一部） | 同上 | なし | ブランチ名・起点・既にあるブランチ・`.worktreeinclude`。絵文字名・repo ごとの base・準備の run / skip・sparse・進捗 / キャンセル / Retry・どこで動かすかは未 |
...
- **計画の続き**: O5（手元の Ports）・O21 の一部（外部の worktree）・O26（エディタの所作・全部）・
  ...
  O22（リソース・片付け）・O23 の一部（fan-out）・O14（アカウント切替）・O16 の一部（レシピ）。O15・O18 は Mac の worktree に途中があるので触っていない（push されれば取り込める）。
