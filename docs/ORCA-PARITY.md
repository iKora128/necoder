...
| O17（一部） | 同上 | なし | 質問カードの自由入力（Other 欄に書いて答える・選んだ案に添える）。スマホの質問の入力欄・サブエージェントの子行・message rail・ツールのまとめ表示と個別停止・Codex の goal は未 |
...
- **計画の続き**: O5（手元の Ports）・O21 の一部（外部の worktree）・O26（エディタの所作・全部）・
  ...
  O17 の一部（質問カードの自由入力）。O15・O18 は Mac の worktree に途中があるので触っていない（push されれば取り込める）。
