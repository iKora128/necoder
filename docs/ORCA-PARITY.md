...
| O27（一部） | 同上 | #30 | 設定の検索（全ページから一致した行を集める・キー / 選択肢の名前でも当たる）・表示言語の切替（その場で）・UI とコードの書体（`ui_font_family` / `code_font_family`・settings.json・直書きしていた 30 か所を `ui::ui_font` / `ui::code_font` に）。・statusbar の項目の出し入れ（右クリック・`statusbar_hidden`）。キー割り当ての GUI・UI ズーム・density・書体を選ぶ画面は未 |
