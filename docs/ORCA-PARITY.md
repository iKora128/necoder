...
- **計画の続き**: O5（手元の Ports）・O21 の一部（外部の worktree・絞り込み・コピー・ピンの保存）・O26（エディタの所作・全部）・
  ...
  O17 の一部（質問カードの自由入力・サブエージェントの手順）・O27 の一部（設定の検索・表示言語）。O15・O18 は Mac の worktree に途中があるので触っていない（push されれば取り込める）。
