...
| O13（一部） | 同上 | なし | 作業中はスリープさせない・通知の履歴（ベル）・通知音の大きさ・ニュースのクリック移動。スマホへの完了 push・メニューバー・TCC の案内は未 |
...
| O17（一部） | 同上 | なし | 質問カードの自由入力（Other 欄に書いて答える・選んだ案に添える）・サブエージェントの手順を親の Task の下に畳む。スマホの質問の入力欄・message rail・ツールのまとめ表示と個別停止・Codex の goal は未 |
...
- **計画の続き**: O5（手元の Ports）・O21 の一部（外部の worktree）・O26（エディタの所作・全部）・
  O20 の一部（ブランチ名・起点・`.worktreeinclude`）・O13 の一部（スリープ抑止・通知の履歴・音量）・
...
| B19 | 構造化質問カード | 有 | 済（自由入力も O17 で済。スマホの入力欄は未） |
