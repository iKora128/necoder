...
エディタとしての深さ（LSP・hunk・blame・⌘I）、ACP による承認と巻き戻し、軽さ、プライバシー、SSH の作りは necoder が勝っている。
...
| 左バーでセッション管理（Herdr 相当） | Fleet サイドバー・herd・要対応キュー。状態は ACP 由来で誤検出が無い（ここは優位）。フィルタ・一括操作・並べ替えは Orca が上（A11 / A13 / A14） | 有 |
...
- **軽い。**
  - GPUI ネイティブで、idle RSS は 122MB、起動は約 215ms（2026-07-17 実測）。WebView は必要な時だけ作って 15 分で捨て、端末は出力がある時だけ起きる。
