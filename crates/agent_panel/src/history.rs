...
/// 「以前の N 件は省略」の区切りを置く。サブエージェントのツールは live と同じく、親の手順が先に
/// 在る時だけ親の下に畳む（親が上限の外なら普通の手順として出す・O17）。相関 id は親子を結ぶために残す。
        ...
        entries.push(match item {
            ...
            ReplayItem::Tool(mut info) => {
                if info
                    .parent
                    .as_deref()
                    .is_some_and(|parent| !steps.contains(parent))
                {
                    info.parent = None;
                }
                if !info.id.is_empty() {
                    steps.insert(info.id.clone());
                }
                build_step_entry(info)
            }
        });
    }
    entries
...
mod tests {
    ...
    /// サブエージェントのツールは、親の手順が先に在る時だけ親の下に畳む（live と同じ・O17）。
    #[test]
    fn replayed_items_become_transcript_entries() {
        let tool = |id: &str, title: &str, parent: Option<&str>| {
        ...
        let items = vec![
            ...
            tool("toolu_1", "Task 調べる", None),
            tool("toolu_2", "Read README.md", Some("toolu_1")),
            tool("toolu_3", "Read orphan.rs", Some("gone")),
        ...
        assert_eq!(
            ...
            vec![
                ...
                "step:Task 調べる:-:# necoder".to_string(),
                "step:Read README.md:toolu_1:# necoder".to_string(),
                "step:Read orphan.rs:-:# necoder".to_string(),
                "agent:直しました。".to_string(),
            ],
            "親が居るサブエージェントの手順は親に付き、居ない物は普通の手順になる"
        );
