...
//!   ツールの呼び出し」へ畳む。思考・プラン・使用量は捨てる。サブエージェントの内側（claude の
...
use crate::{
    initialize_request, map_tool_kind, tool_completed, tool_diffs, tool_locations, tool_output,
    with_handshake_timeout, with_timeout, AgentCommand, ToolCallInfo,
};
use acp::schema::v1;
    ...
    pub fn push(&mut self, update: v1::SessionUpdate) {
        match update {
            ...
            v1::SessionUpdate::ToolCall(tool_call) => {
                if is_subagent(tool_call.meta.as_ref()) {
                    return;
                }
                self.open_text = None;
...
/// サブエージェントの内側の更新か（主の会話の transcript には載せない）。
...
mod tests {
    ...
    fn replayed_updates_fold_into_turns_and_tool_calls() {
        ...
        // サブエージェントの内側（claude）: 主の transcript には出さない。
        log.push(update(json!({
